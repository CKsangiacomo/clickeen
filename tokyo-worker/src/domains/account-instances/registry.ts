...
function assertScopedIds(
  accountIdRaw: string,
  instanceIdRaw?: string,
): { accountId: string; instanceId?: string } {
  ...
  if (
    ...
    (instanceIdRaw != null && !isCompactInstanceId(instanceId || ''))
  ) {
...
export async function readInstanceRegistryRow(args: {
  ...
  const payload = await assertSupabaseOk(
    await supabaseFetch(
      args.env,
      `/rest/v1/instances?account_id=${eq(accountId)}&id=${eq(instanceId!)}&select=${select}&limit=1`,
    ),
    'read_failed',
  );
  ...
  if (!row) throw new Error('tokyo.instance.registry.read_invalid');
  return row;
}
...
export async function createInstanceRegistryRow(args: {
  ...
  if (!widgetType || !resolveWidgetCode(widgetType))

...
async function prepareContext(config) {
    ...
    const widgetId = widgetResponse[0]?.id ?? widgetResponse.id;
    if (!widgetId) {
...
async function testCorsS2S(config, context) {
    ...
    if (!body || body.status !== 'published') {
        throw new Error('Response missing published status confirmation');
    }
    return {
        metrics: {
            status: res.status,
...
async function testPutStatusOnly(config, context) {
    ...
    if (!body || body.status !== 'draft') {
        throw new Error('Response did not return updated draft status');
    }
    return {
        metrics: {
            status: res.status,
...
async function testDbIndexes(config) {
    const headers = defaultSupabaseHeaders(config);
    const widgetIdxUrl = new URL('/rest/v1/pg_indexes', config.supabaseUrl);
    widgetIdxUrl.searchParams.set('select', 'indexname,indexdef');
    widgetIdxUrl.searchParams.set('schemaname', 'eq.public');
    widgetIdxUrl.searchParams.set('tablename', 'eq.widget_instances');
    widgetIdxUrl.searchParams.set('indexname', 'eq.idx_widget_instances_widget_id');
    const usageIdxUrl = new URL('/rest/v1/pg_indexes', config.supabaseUrl);
    usageIdxUrl.searchParams.set('select', 'indexname,indexdef');
    usageIdxUrl.searchParams.set('schemaname', 'eq.public');
    usageIdxUrl.searchParams.set('tablename', 'eq.usage_events');
    usageIdxUrl.searchParams.set('indexname', 'eq.usage_events_idempotency_hash_key');
    const widgetIdx = await supabaseRequest(widgetIdxUrl.toString(), { method: 'GET', headers }, 'Failed to verify widget index');
    const usageIdx = await supabaseRequest(usageIdxUrl.toString(), { method: 'GET', headers }, 'Failed to verify usage index');
    if (!Array.isArray(widgetIdx) || widgetIdx.length !== 1) {
        throw new Error('idx_widget_instances_widget_id is missing');
    }
    if (!Array.isArray(usageIdx) || usageIdx.length !== 1) {
        throw new Error('usage_events_idempotency_hash_key is missing');
    }
    return {
        metrics: {
            widgetInstancesIndex: widgetIdx[0].indexdef,
            usageEventsIndex: usageIdx[0].indexdef,
        },
    };
}

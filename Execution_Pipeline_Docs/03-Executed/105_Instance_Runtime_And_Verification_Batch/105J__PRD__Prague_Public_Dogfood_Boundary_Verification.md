...
- Prague authored content uses explicit account instance refs with `accountPublicId` and `instanceId`.
- `pnpm prague:blocks:validate` passes.
- `pnpm prague:l10n:verify` passes.
- `pnpm --filter @clickeen/prague build` passes.
- Scans found no active Prague dependency on private translation operation state or default per-locale HTML/JS public artifact shapes.

Strict public embed resolution against deployed `clk.live` remains a cloud/live verification concern.

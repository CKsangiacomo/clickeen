...
Source:
- Berlin deep-auth analysis findings, folded into this executed PRD
...
- Final cloud-dev smoke becomes manual Google login, Roma bootstrap, Builder open/save, Widgets/listed-instance read, account locale read/write, assets path read/write, and logout.
...
- Widgets/listed-instance/Builder/account-locale/assets paths that previously touched Michael still work through the named surviving owner.
...
### 2026-04-30 Cut 5 Final Correction

Status: GREEN

The temporary repair kept login working, but it restored the application-level identity race handler that this PRD explicitly deleted. That was not acceptable as final architecture.

Final repair:

- Berlin login again calls `/rest/v1/rpc/resolve_login_identity`.
- The app-level `login_identities` read/write/reread conflict handler is deleted.
- The existing `20260427073500__prd77_reload_resolve_login_identity_rpc.sql` migration remains the schema-cache reload guard for the RPC.
- Missing RPC/function state is now a hard database-boundary failure, not a reason to revive the old application race code.

Deletion proof:

- Deleted `loadLoginIdentity`.
- Deleted `writeLoginIdentity`.
- Deleted `loadExistingUserProfile`.
- Deleted `upsertUserProfile`.
- Deleted `resolveNewProductUserId`.
- Deleted raced-identity/raced-profile retry branches.
...
### 2026-04-30 Final Closure

Status: GREEN

Final proof:

- Old implicit session mode is gone.
- Marker-only Michael bridge is gone.
- V1 refresh compatibility is gone.
- Public password login is gone.
- Direct-provider email change is explicit/hidden from Roma product UI.
- DB-owned provider identity resolution is restored through `resolve_login_identity`.
- Duplicate profile normalization remains consolidated.
- Berlin route dispatch is table-based and contains only the surviving product/auth routes.
- Docs and runtime agree on no Berlin/Roma product dependency on `CK_INTERNAL_SERVICE_JWT`.

Verification:

- `rg "internal_service_role|isTrustedRomaMichaelTokenRequest|sec-fetch-site|sec-fetch-mode|sec-fetch-dest|resolveMichaelAccessToken|createMichaelHeaders|fetchMichaelListRows|/auth/michael/token|auth/login/password|NEXT_PUBLIC_ROMA_PASSWORD_LOGIN|requestSupabasePasswordGrant|CLOUD_DEV_SMOKE_EMAIL|CLOUD_DEV_SMOKE_PASSWORD|supabase_bridge|SupabaseBridge|ensureSupabaseAccessToken|RefreshPayloadV1|payload\\.v === 1|version === 1|supabaseRefreshToken|Boolean\\(.*supabaseRefreshToken|hasSupabaseSessionBridge" berlin/src roma/app documentation/services documentation/architecture .github/workflows -g '!node_modules'` -> empty
- `rg "loadLoginIdentity|writeLoginIdentity|loadExistingUserProfile|upsertUserProfile|resolveNewProductUserId|racedIdentity|racedProfile|on_conflict.*login_identities|/rest/v1/login_identities\\?" berlin/src/account-reconcile.ts -g '!node_modules'` -> empty
- `corepack pnpm exec tsc -p berlin/tsconfig.json --noEmit` -> PASS
- `corepack pnpm exec tsc -p roma/tsconfig.json --noEmit` -> PASS
- `PATH="/tmp/clickeen-pnpm-shim:$PATH" corepack pnpm lint` -> PASS
- `PATH="/tmp/clickeen-pnpm-shim:$PATH" corepack pnpm typecheck` -> PASS
- `COREPACK_ENABLE_DOWNLOAD_PROMPT=0 NEXT_PUBLIC_TOKYO_URL=https://tokyo.dev.clickeen.com corepack pnpm --filter @clickeen/roma build` -> PASS
- `git diff --check` -> PASS
...
- Smoke Widgets/listed-instance, Builder open/save, account locales, and assets paths that used to touch Michael.

...
- `tokyo-worker/src/domains/translation-state.ts` is deleted.
- No active code references `translation-state`, `state/current.json`, or `l10n/instances/<publicId>/state/current.json`.
- Only documentation states that the separate translation-state file does not exist on the product path.

Verification:

- `corepack pnpm exec tsc -p tokyo-worker/tsconfig.json --noEmit` -> PASS
- `corepack pnpm exec tsc -p bob/tsconfig.json --noEmit` -> PASS
- `git diff --check` -> PASS

Result:

- PRD 078 is fully closed in code and docs.

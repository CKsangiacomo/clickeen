...
Status: Executed historical evidence / surviving doctrine extracted to PRD 105A; superseded by PRD 105 and 105A where conflicting
Date: 2026-05-21
...
Archive note: this execution ledger is no longer the active 103-family execution item. It is retained as evidence of DB Pivot slices and status at the time. Current execution authority starts from PRD 105 and PRD 105A.
...
| 103_DB.5 - Translation generation control state | Red - requires 103K repair | Keep coarse `instances.translation_status`, but replace the fragile random-job current-generation model with PRD 103K saved-base-content sync. Current artifact: `103_DB_Translation_Generation_Control_State__EXEC__Tokyo_Runtime_Wiring.md`. | Green only after 103K proves active Generate is idempotent by saved base content marker, completion applies by marker match, and Bob shows out-of-sync state correctly. |
...
| DB-007 | P0 | Translation generation job | Object-backed job state, random job-id authority, and local spinner inference | Tokyo Generate operation plus `instances.translation_status` and PRD 103K saved-base-content sync marker; no job table by default | 103_DB.5 / 103K | Red |
| DB-008 | P1 | Translation queue completion | SF completion can be discarded because a duplicate Generate replaced the current random job id | Tokyo completion applies by saved base content marker through product operation | 103_DB.5 / 103K | Red |
...
Current executable slice is blocked by `103K - Saved Base Content Translation Sync`.

Current 103J/103K readout:

- 103J remains the generic widget text contract authority.
- 103K is now the translation sync authority.
- Red: current runtime can discard valid translated output as stale when duplicate Generate creates a new random current job.
- Blocked: authenticated Roma cloud-dev smoke cannot close until 103K runtime wiring proves edit/save, out-of-sync banner, Generate disabled while active, translated preview, and publish for FAQ plus at least one non-FAQ widget.

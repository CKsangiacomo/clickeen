...
Executed as a deletion/simplification pass:

- Kept explicit `show-if` because active widget specs still depend on it.
- Deleted the hidden `autoNestShowIfDependentClusters` DOM re-parenting behavior from Bob runtime.
- Removed unused `contains(...)` show-if function support.
- Collapsed duplicate compiler JSON attribute normalization into one helper.

Verification:

- `./node_modules/.bin/tsc -p bob/tsconfig.json --noEmit`
- Residual search confirmed the deleted auto-nesting helper and duplicate normalization names are gone.

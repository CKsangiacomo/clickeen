...
Verified before moving to executed:

- Builder account asset commands route through one host command family: `list-assets`, `resolve-assets`, `upload-asset`.
- Roma maps those commands to one account route family: `/api/account/assets`, `/api/account/assets/resolve`, `/api/account/assets/upload`.
- Dieter has one shared account asset client at `dieter/components/shared/account-assets.ts`.
- No `hostedAssetBridge`, `asset-picker-overlay`, `asset-picker-data`, `assetResolve`, `assetUpload`, or `bob:asset-entitlement-denied` product path remains.
- Asset upload/list/resolve/delete all stay account-owned and Tokyo-worker-backed.
- Asset identity remains immutable: controls persist `assetId`/metadata, preview resolves delivery URLs separately.

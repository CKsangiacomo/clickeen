...
- Berlin `readSupabaseAdminListAll` lives in `berlin/src/supabase-admin.ts`; `berlin/src/supabase-list.ts` is gone.
- Berlin HTTP response helpers live in `berlin/src/http.ts`; `berlin/src/helpers.ts` keeps token/binary/claim helpers only.
- Tokyo-worker HTTP/request observation lives in `tokyo-worker/src/http.ts`; `tokyo-worker/src/request-ops.ts` is gone.
- Tokyo-worker account-localization barrel is gone.
- Tokyo-worker render logic is split under `tokyo-worker/src/domains/render/`.
- The stale one-line compatibility shim `tokyo-worker/src/domains/render.ts` was deleted in this pass.
...
- No active `supabase-list` imports.
- No Tokyo-worker `request-ops` imports.
- No Tokyo-worker `domains/account-localization` barrel imports.
- `tokyo-worker/src/domains/render.ts` does not exist; `tokyo-worker/src/domains/render/` does.
- HTTP helpers are absent from `berlin/src/helpers.ts`; token/binary helpers are absent from `berlin/src/http.ts`.
- `./node_modules/.bin/tsc -p berlin/tsconfig.json --noEmit`
- `./node_modules/.bin/tsc -p tokyo-worker/tsconfig.json --noEmit`
- `./node_modules/.bin/wrangler deploy --dry-run --outdir /tmp/clickeen-berlin-hygiene-dryrun` from `berlin/`
- `./node_modules/.bin/wrangler deploy --dry-run --outdir /tmp/clickeen-tokyo-hygiene-dryrun` from `tokyo-worker/`
- `git diff --check`
- Live smoke:
  - `https://berlin-dev.clickeen.workers.dev/internal/healthz` returned `200`
  - `https://tokyo.dev.clickeen.com/healthz` returned `200`

...
- San Francisco no longer uses the old generic `/execute` route as the agent
  brain path.
...
- Future public locale serving is explicitly outside 121D closure.
- Account settings active-locale reconciliation is inside 121D closure and is
  implemented through Roma, Translation Agent, and Tokyo-worker.
- Runtime docs for the active path match the executed code.
...
```text
Bob/Roma -> Product Copilot Worker -> San Francisco /model/chat -> Bob
```
...
- San Francisco `/execute` returns the expected visible 410 hard cut.
...
Product Copilot and Translation Agent paths work.

Execution result:

- no product dependency remains on San Francisco as the saved-instance
  translation owner;
- active runtime docs describe San Francisco as model execution only;
- active runtime docs keep account authorization in Roma, not San Francisco;
- the current 121D/completeness docs avoid broad tool/router/fallback language;
- no compatibility wrapper was added to preserve old fake-agent shapes.

### Slice E - Closure Evidence

This PRD moved to executed after evidence existed:
...
121 through 121D completeness is executed with:
...
- Product Copilot runtime path complete;
- 121D Translation Agent runtime path complete on cloud-dev;
- account settings active-locale reconciliation updating overlay files through
  the Roma settings route;
- Translation Agent Worker home present;
- redundant runtime discovery and compatibility paths avoided or removed where
  present in current product code;
- active docs keeping future agent/serving work outside current-system truth.

Docs:

- `CONTEXT.md` remains current-system only;
- Roma, Bob, Tokyo-worker, San Francisco, and Translation Agent docs describe
  the current runtime path;
- Translation Agent implementation detail remains in 121D;
- future ranking/measurement and serving work remain future scope.

Evidence:

- focused developer commands for touched surfaces were run;
- behavior was proved through owning product routes;
- runtime result was recorded after code worked;
- no-fallback behavior was recorded where it can be proved without mutating live
...
- Product Copilot runtime smoke is green through
  `pnpm e2e:smoke:copilot-runtime` on 2026-06-23. The smoke returned
  Product Copilot request id `e420996c-3471-49aa-9738-b1bbfc0b67e6` and proved
  unmanaged selected model `openai:not-a-managed-model` is rejected with
  HTTP 422.
- Translation Agent generation was proved on `QD1G068MX7` in account
  `CLICKEEN` through `pnpm e2e:smoke:translation-agent-runtime` on 2026-06-23:
  Bob clicked
...
Executed on 2026-06-23 as 121-through-121D runtime completeness closure.

Closed runtime facts:

- Product Copilot is a real Worker agent home and proved through the cloud-dev
  product path.
- Translation Agent is a real Worker agent home and proved through the
  cloud-dev product path.
- San Francisco is model execution only for the active agent paths; `/execute`
  remains hard-cut.
- Bob/Roma invoke the named agent homes instead of hiding agent work inside
  product endpoints.
- Roma owns account, tier, active locales, route, and save.
- Tokyo-worker stores exact account runtime files and overlay files.
- No future clk.live locale serving or SEO/GEO/AEO work was smuggled into this
  closure.
...
Product Copilot cloud-dev smoke result:

- account: `CLICKEEN`;
- instance: `QD1G068MX7`;
- route request id: `e420996c-3471-49aa-9738-b1bbfc0b67e6`;
- result kind: `answer`;
- no-fallback proof: unmanaged selected model
  `openai:not-a-managed-model` rejected with HTTP `422`;
- Bob Builder opened `https://roma.dev.clickeen.com/builder/QD1G068MX7`.

Translation Agent cloud-dev smoke result:

- account: `CLICKEEN`;
- instance: `QD1G068MX7`;
- base locale: `en`;
- active locale count: `28`;
- generated locale count: `28`;
- sampled locale: `ja`;
- sampled overlay value count: `5`;
- Bob Builder opened the deployed instance and rendered saved translated overlay
  values.

Core violation audit:

- Silent substitution: pass. No fallback model, provider, locale, route, or
  artifact substitution was found in the active 121D/completeness path.
- Silent healing: pass. Translation Agent evals reject malformed output
  rather than repairing it.
- Silent omission: pass. Runtime smokes prove Product Copilot and Translation
  Agent product paths, not direct brain-only tests.
- Fail-open control: pass. Unmanaged Product Copilot model selection returns
  HTTP `422`; Translation Agent invalid outputs fail explicitly.
- Corruption-as-absence: pass. Overlay writes validate against saved source
  field paths.
- Partial-success masquerade: pass. Roma requires all requested Translation
  Agent locale results to succeed before accepting generation.
- Masquerade/redress: pass. Active paths use agent homes; San Francisco does
  not execute agent brains through `/execute`.
- Runtime test dependency: pass. E2E smokes and evals are evidence only and
  are not runtime product dependencies.

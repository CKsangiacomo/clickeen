...
- Translation Agent Worker verifies the Roma signed grant, calls San Francisco
  `/model/chat` through a service binding, and writes locale overlays through
...
-> Translation Agent Worker calls San Francisco /model/chat
...
- San Francisco `/model/chat` client;
...
The Worker path is the product path. Roma calls Translation Agent for saved
account-widget translation generation. San Francisco remains the governed model
execution boundary through `/model/chat`; it is not the saved-instance
translation owner.
...
6. Call San Francisco `/model/chat` with the Roma-issued grant.
...
1. Serve governed `/model/chat` calls from the Translation Agent Worker.
...
This must be enforced in `/model/chat` for Translation Agent and covered so
Product Copilot does not regress.

No product dependency on a San Francisco saved-instance translation owner
remains in 121D. Prague system-copy translation tooling remains separate and is
not the account-widget Translation Agent path.
...
Bob does not show backend lifecycle state, raw reason keys, or locale authority
details in this panel.

The in-flight count is only the active locale count Bob already received in the
open editor payload. Bob does not create a backend task, poll backend state,
stream a counter, or invent another locale authority.
...
Closure status: Translation Agent Worker home exists and Roma invokes it from
the product generation route. Bob's Translations panel uses account active
locales for language selection, sends only the instance generation command to
Roma, shows running state while the operation is in flight, shows the generated
active-locale count after Roma returns, and does not run save-follow-up
generation state, polling, jobs, or backend status machinery. Runtime public
locale consumption and public page/runtime consumption remain later work.
...
Audit completed on 2026-06-23. Result: GREEN. Details are recorded in
Execution Notes.

## 19. Execution Notes

Executed on 2026-06-23 as the current Translation Agent runtime closure.

Code/runtime result:

- Translation Agent is a Cloudflare Worker home under `agents/translation-agent`.
- Roma generation route reads account active locales, removes base locale, and
  calls the Translation Agent Worker; Bob sends no locale authority.
- Translation Agent calls San Francisco `/model/chat` and writes exact locale
  overlay files through Tokyo-worker.
- Bob Translations panel is one explicit Generate translations operation,
  disables during the request, shows the generated active-locale count, refreshes
  overlay inspection, and previews saved overlay values.
- Account settings active-locale save reconciles saved instance overlays through
  Roma: added locales generate, removed locales delete, unchanged locales do
  nothing.
- Public locale/page consumption remains future runtime work.
...
Cloud-dev runtime evidence from `pnpm e2e:smoke:translation-agent-runtime`:
...
- Silent substitution: pass. Bob/Roma do not substitute locale authority;
  generated locale count comes from Translation Agent response.
- Silent healing: pass. Malformed Translation Agent output fails in evals and
  Roma rejects invalid response shape.
- Silent omission: pass. Translation Agent eval covers missing paths and
  runtime smoke generated all active non-base locales.
- Fail-open control: pass. Missing/invalid model route is not bypassed by
  Translation Agent or San Francisco.
- Corruption-as-absence: pass. Overlay validation uses saved content field
  paths and rejects malformed value maps.
- Partial-success masquerade: pass. Roma requires every requested active
  locale result to succeed before returning accepted generation.
- Masquerade/redress: pass. Product generation path is the Translation Agent
  Worker, not San Francisco's old saved-instance translation route.
- Runtime test dependency: pass. Smokes/evals are verification only; product
  runtime does not depend on jobs or test helpers.

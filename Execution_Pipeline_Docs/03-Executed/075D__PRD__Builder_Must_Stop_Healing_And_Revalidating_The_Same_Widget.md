...
`075D` is closed in code.

Verified current state:

- Bob session boot clones the saved config it receives and does not merge defaults into it.
- Bob save sends the active widget document as-is through the Roma host command.
- The deleted `normalizeSessionConfig` path is not referenced by Bob.
- Roma opens and saves through the Tokyo saved-document boundary instead of replacing local truth with a separate repaired document.
- Tokyo saved-document reads fail at the Tokyo boundary when the stored pointer/config is invalid.
...
- `rg "normalizeSessionConfig|mergeMissingDefaults" bob`

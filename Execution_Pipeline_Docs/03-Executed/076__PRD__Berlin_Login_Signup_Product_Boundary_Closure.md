...
Closed by 2026-04-30 follow-up:

- Roma `/api/session/login/google` now redirects the browser directly to Berlin `GET /auth/login/google/start`.
- Berlin no longer exposes the generic JSON start route or generic provider callback route.
- `BERLIN_LOGIN_CALLBACK_URL` and Supabase Auth provider callback allow-list residue were deleted from checked-in config/docs.

## 2026-04-24 Post-Login Michael Bridge Patch - Superseded
...
- This residual bridge was deleted by PRD 077. The surviving owner for account registry/product reads is Berlin's explicit account registry endpoints; Roma no longer receives a Michael/PostgREST token from Berlin.

## 2026-04-30 Final Closure

Implemented:

- Roma Google start route became a pure redirect to Berlin's direct Google route.
- Berlin deleted `POST /auth/login/provider/start` and `GET /auth/login/provider/callback`.
- Berlin deleted the JSON provider-start handler.
- Berlin deleted `BERLIN_LOGIN_CALLBACK_URL` from runtime env typing and Wrangler config.
- Supabase local config no longer allow-lists the old provider callback URLs.
- Berlin service docs now list only the surviving direct-provider auth routes.

Verification:

- `rg "auth/login/provider/start|auth/login/provider/callback|BERLIN_LOGIN_CALLBACK_URL|handleProviderLoginStart|resolveLoginCallbackUrl|auth/v1/authorize|requestSupabaseOAuthUrl|NEXT_PUBLIC_ROMA_PASSWORD_LOGIN|auth/login/password|CLOUD_DEV_SMOKE_EMAIL|CLOUD_DEV_SMOKE_PASSWORD" berlin roma documentation supabase scripts .github -g '!node_modules'` -> empty
- `corepack pnpm exec tsc -p berlin/tsconfig.json --noEmit` -> PASS
- `corepack pnpm exec tsc -p roma/tsconfig.json --noEmit` -> PASS
- `git diff --check` -> PASS
...
- PRD 076 is fully closed in code and docs.

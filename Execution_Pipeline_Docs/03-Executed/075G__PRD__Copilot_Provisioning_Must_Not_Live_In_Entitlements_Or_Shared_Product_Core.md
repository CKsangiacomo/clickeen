...
- Deleted AI provisioning from `@clickeen/ck-policy`; entitlement truth now stays in policy, while AI registry/provider/model truth lives in `@clickeen/ck-contracts`.
- Removed Bob account Copilot provider/model/agent selection. Bob sends only the Copilot prompt and current editor context.
- Roma now chooses the one account Builder Copilot agent internally (`cs.widget.copilot.v1`) and ignores client-side product-variant knobs.
- Verified with Bob, Roma, San Francisco, and Admin TypeScript checks.

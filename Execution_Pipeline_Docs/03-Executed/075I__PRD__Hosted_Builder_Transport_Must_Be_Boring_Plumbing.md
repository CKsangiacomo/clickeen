...
- Removed unused Bob transport state plumbing and stale route-shaping parameters.
- Kept the real iframe mechanics: Bob ready signal, one Roma open payload, and Bob open confirmation/error so invalid opens fail visibly.
- Updated architecture docs to describe the current hosted Builder flow without stale `boot=message` or transport-protocol wording.
- Verified with Bob and Roma TypeScript checks plus residual text search for stale transport language.

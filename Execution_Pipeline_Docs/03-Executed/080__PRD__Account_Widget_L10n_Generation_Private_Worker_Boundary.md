...
### Execution Closure - 2026-04-30

Executed against the current codebase.

What is now true:

- Tokyo-worker calls San Francisco through the private `SANFRANCISCO_L10N` WorkerEntrypoint method `generateAccountWidgetL10nOps`.
- San Francisco does not expose public `POST /v1/l10n/account/ops/generate`.
- Account-widget l10n generation does not use `CK_INTERNAL_SERVICE_JWT`, `SANFRANCISCO_BASE_URL`, `TOKYO_L10N_HTTP_BASE`, or a public HTTP fallback.
- Tokyo-worker sends San Francisco only approved current text items, existing ops, changed paths, removed paths, target locales, widget type, base locale, and policy profile.
- San Francisco account-widget generation does not receive widget config or allowlists.
- Account-widget l10n states are `queued`, `working`, `ready`, and `failed`.
- Failed queue handoff/exhaustion writes `failed` to the saved pointer with failed locale reasons.
- The stale Roma `CK_INTERNAL_SERVICE_JWT` helpers were deleted.
- The stale Tokyo l10n HTTP bridge helpers and `TOKYO_L10N_HTTP_BASE` dev var were deleted.
- The stale private `/__internal/l10n/instances/:publicId/translations` route was replaced by `/__internal/account/instances/:publicId/translations`.
- Public `/l10n/instances/**` survives only as serving/projection reads; stale public write guards were removed.

Verification:

- `rg "generateLocaleOpsWithSanfrancisco|/v1/l10n/account/ops/generate|TOKYO_L10N_HTTP_BASE|resolveL10nHttpBase|buildL10nBridgeHeaders|x-tokyo-l10n-bridge|resolveInternalServiceJwt|resolveSanfranciscoInternalToken" tokyo-worker/src sanfrancisco/src roma scripts documentation`
- `rg "CK_INTERNAL_SERVICE_JWT" tokyo-worker/src/domains tokyo-worker/src/routes roma`
- `rg "\baccepted\b" tokyo-worker/src roma bob packages`
- `rg "allowlist|config" sanfrancisco/src/l10n-account-routes.ts`
- Incremental merge proof: one changed item preserves untouched existing locale ops.

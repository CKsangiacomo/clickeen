...
**User-facing change:** Yes (major; introduces the central authenticated Roma app for all product domains)
...
Roma is not "Bob renamed." Roma is the **Clickeen App** where users manage:
...
**Core decision:** keep existing backend boundaries (Paris, Tokyo Worker, Venice, San Francisco, Prague), and add a first-class Roma layout and Roma app surface that orchestrates these domains coherently.
...
### 1.1 Current app surface is editor-centric, not Roma app

1. Root Bob page is a launcher, not a Roma layout:
...
### 1.2 Architecture is explicitly multi-system, so a central Roma app is missing
...
2. Production mapping currently points Bob to `app.clickeen.com`, which conflates deployment hostname with product Roma app identity:
...
2. Growth beyond Phase A app ergonomics is blocked by missing Roma app architecture.
...
We currently lack a dedicated **authenticated product Roma app** that composes all existing system capabilities under one coherent application model.
...
4. **Scalability drag:** adding outputs (emails/pages/creatives) will multiply UX/infra fragmentation if no central Roma layout exists.
...
1. Establish **Roma** as the canonical internal system for Clickeen App Roma app concerns.
2. Keep Bob as the **builder module** inside Roma (not the entire app identity).
3. Provide first-class modules for account/workspace/billing/user/assets/widgets/usage management.
...
4. No pre-implementation gold-plating outside Roma app scope.
...
2. **Orchestrators remain dumb pipes:** no widget-specific healing in Roma app layers.
...
6. **One Roma app, many domains:** Roma must orchestrate, not absorb all domain logic.
7. **MiniBob and Roma are separate lifecycle tracks:** MiniBob artifacts are temporary/non-canonical; Roma owned instances are canonical and are always materialized as new records during MiniBob continuation (never rebound/moved).
...
1. **Roma** (Cloudflare Pages / Next.js): authenticated Roma app + module router + same-origin API proxy.
...
5. Centralized UX primitives for Roma app flows (tables, filters, asset manager, billing state, invites, audit views).
...
3. `widgets` - widget instance library (user + curated visibility where policy allows)
...
3. `/widgets` -> widget instances grid/list
4. `/widgets/:publicId` -> widget instance details
...
6. No redirect compatibility layer: builder launches must use the Roma route contract directly in all environments.
7. No separate long-term app identity for Builder.

### 7.4 Roma Layout Contract (Locked UX direction)
...
1. Roma follows a Dropbox-like high-level layout/information architecture:
   ...
   - top command bar for workspace/account context and primary actions
...
3. `home`, `widgets`, `assets`, `team`, `billing`, `usage`, `ai`, and `settings` share one Roma layout contract.
4. Builder route keeps shared app identity but supports focus mode:
   - Roma layout-level breadcrumbs + account/workspace context remain available
...
3. Query-param subject overrides are disabled in production Roma app flows.
...
This section defines required schema capabilities for Roma app completeness.
...
#### A) Account membership and Roma app ownership
...
Roma requires contract-complete API surfaces for Roma app operations.
...
7. `POST /api/minibob/handoff/start`
   - stores MiniBob snapshot server-side and returns opaque `handoffId`
8. `POST /api/minibob/handoff/complete`
   - consumes `handoffId` and deterministically materializes a new Roma-owned instance using stored snapshot context
...
Roma UI route naming is `widgets`; Paris API path naming remains `instances` in this slice.
...
5. Only after explicit account + workspace context exists can user enter builder/Roma app modules.
6. No hidden auto-provisioning and no silent fallback account/workspace creation.

### 12.1 Smooth signup + MiniBob draft handoff from Prague to Roma/Bob

1. User starts in MiniBob on Prague with draft context for a widget (`publicId` + optional draft config), where `publicId` is a curated/base source (`wgt_main_*` or `wgt_curated_*`), never a user instance id.
2. Publish intent triggers signup/auth only if needed.
3. Prague calls handoff start endpoint to persist snapshot server-side and receives opaque `handoffId`.
4. Prague redirects to Roma with `handoffId` (no signed payload token in URL).
5. If user has no account/workspace yet, Roma runs explicit bootstrap (Section 12.0) while preserving pending handoff state.
6. After account/workspace selection/creation, Roma calls handoff completion with `handoffId` automatically.
7. Handoff completion endpoint is idempotent (`Idempotency-Key` + consumed-state replay) so retries and refreshes are safe.
8. Paris creates a new canonical `widget_instances` row in target workspace and returns canonical builder route.
...
### 12.2 Create widget in Roma

1. User opens `/widgets`.
...
1. Workstream A is the Roma layout prerequisite for all other workstreams.
...
Create new `roma/` app package and route Roma layout.

### Why

Separate app identity from builder identity and establish Roma app backbone.
...
1. Roma runs locally with auth-guarded route Roma layout.
...
Roma app requires production-grade identity semantics.

### How

1. Implement `GET /api/me` and membership resolution.
2. Implement Supabase JWT verification path in Paris for Roma app endpoints.
3. Add account/workspace role checks in Paris for Roma app endpoints.
...
Billing is a core Roma app domain and must not be implicit/internal.
...
Roma app without team controls is incomplete and blocks collaboration scale.
...
AI is core platform behavior; Roma app must make it observable and governable.
...
| `POST /api/minibob/handoff/start` | none (Prague server relay) | `public` | `minibob.handoff.start.req.v1` / `minibob.handoff.start.res.v1` | no | `minibob.handoff.started` |
| `POST /api/minibob/handoff/complete` | Supabase session JWT + `handoffId` | `authenticated` | `minibob.handoff.complete.req.v1` / `minibob.handoff.complete.res.v1` | yes (`Idempotency-Key` + consumed-state replay) | `instance.created_from_minibob_handoff` |
...
2. Roma app API p95 <= 500ms (read) and <= 800ms (write)
...
1. Elevated 5xx rate on Roma app APIs
...
6. Roma layout must implement the Section 7.4 left-rail + top-command-bar + main-canvas pattern across Roma app modules.
7. MiniBob -> Roma -> Bob PLG handoff must preserve draft continuity without manual copy/paste or repeated handoff re-entry.
...
2. `pnpm typecheck`
3. `pnpm test`
4. Roma route integration tests for critical flows.
...
1. Signup -> MiniBob handoff -> builder
...
6. Interrupted PLG flow (refresh/session rotation mid-handoff) resumes to the same pending handoff and lands in `/builder/:publicId` without data loss.
7. Invalid/expired/already-consumed `handoffId` returns explicit blocking state and causes no ownership mutation.

### 19.3 Rejection criteria

1. Any endpoint with ambiguous authz behavior.
2. Any Roma app flow relying on dev-only secrets in production path.
3. Any silent fallback to legacy behavior.
4. Any MiniBob handoff flow that can drop draft context or require manual state reconstruction.
5. Any handoff path that accepts a user instance `publicId` (`wgt_*_u_*`) as source lineage.
6. Any flow that carries signed draft payloads in URL query params instead of using server-side handoff state.
...
1. Keeps widget semantics in existing widget/runtime contracts (no Roma app widget branching).
...
3. Complexity is concentrated where it belongs: Roma app composition.
...
1. Establishes the missing Roma app needed for product evolution beyond editor-only.
...
It is **Roma**: a coherent Roma app that unifies editing, assets, billing, team, usage, and AI governance while preserving strict multi-system architecture boundaries and pre-GA convergence discipline.

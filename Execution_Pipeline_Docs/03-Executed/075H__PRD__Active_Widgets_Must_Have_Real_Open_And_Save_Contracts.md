...
- Added a small Roma persisted-document contract for the active widgets: `faq`, `countdown`, and `logoshowcase`.
- Wired the contract into Roma open, save, and create boundaries.
- The contract validates structural sanity only when active-widget fields are present; it does not require saved rows to match today’s full `spec.defaults`.
- Verified with Roma/Bob TypeScript checks and a contract smoke check covering valid active configs, a narrow saved row, and an invalid FAQ shape.

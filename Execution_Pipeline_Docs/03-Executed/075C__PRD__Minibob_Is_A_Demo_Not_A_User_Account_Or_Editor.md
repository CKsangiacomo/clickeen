...
- Shared Bob authoring code no longer models `minibob` as a peer subject.
- `packages/ck-policy` profiles are only real account tiers: `free`, `tier1`, `tier2`, `tier3`.
- Bob Minibob editor/session routes are gone from active API files.
- Prague keeps `minibob` only as a marketing block that embeds a live public widget and links to account creation.
- Roma account logic does not branch on Minibob.
- Stale Prague source copy that described editing/publishing in Minibob was replaced with preview/signup language.

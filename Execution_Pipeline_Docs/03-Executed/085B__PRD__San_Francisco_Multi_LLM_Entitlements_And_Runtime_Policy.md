...
`policyVersion` is not a hand-authored string. It is a deterministic canonical hash for the resolved runtime policy tuple: `agentId`, `enabled`, `defaultModel`, `modelsByProvider`, `allowModelPicker`, request ceilings, learning-capture policy, and timeout/cost limits. It must not include live usage counters, user id, account id, request id, or free-form plan labels. Rollback is restoring the previous matrix/catalog inputs, which restores the previous version hash.
...
- product policy mints `learningCapture.captureRawFailures`

...
Executed as a Roma helper-thinning pass:

- Collapsed duplicated Tokyo upstream failure mapping in `account-instance-direct.ts`.
- Collapsed duplicated widget config contract failure shaping in `account-instance-direct.ts`.
- Deleted unused account asset gateway validators.
- Removed Builder open timing/debug-only plumbing from `builder-domain.tsx`.

Verification:

- `./node_modules/.bin/tsc -p roma/tsconfig.json --noEmit`
- Residual search confirmed the deleted validators and debug-only Builder timing symbols are gone.

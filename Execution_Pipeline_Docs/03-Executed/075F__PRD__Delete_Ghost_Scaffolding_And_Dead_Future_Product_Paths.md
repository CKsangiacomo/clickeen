...
`075F` is closed in code.

What changed in this closure pass:

- Removed stale Builder copy that described the Roma account host as a workspace/workspace host.
- Removed stale Copilot copy that told users they were editing in a workspace.
- Deleted the unused `toolCaps` field from the AI registry shape and removed the dead personalization tool-cap list, including the zombie `tool:writeWorkspaceProfile` capability.

Verified current state:

- Bob has no direct fetch path to `/api/account/assets`, `/api/account/assets/resolve`, or `/api/account/assets/upload`.
- `AiExecutionSurface` only advertises real live surfaces: `execute` and `endpoint`.
- `tokenBudgetDay` and `tokenBudgetMonth` are gone.
- `createInitialSessionState` has no fake policy parameter and no `void policy`.
- Curated/product truth was not touched.
- Roma -> Bob hosted Builder remains the only account authoring host story.
...
- `./node_modules/.bin/tsc -p admin/tsconfig.json --noEmit`
- `rg "/api/account/assets|/api/account/assets/resolve|/api/account/assets/upload" bob`
- `rg "AiExecutionSurface.*queue|'queue'|\"queue\"|tokenBudgetDay|tokenBudgetMonth|toolCaps|writeWorkspaceProfile|createInitialSessionState\\([^)]|void policy|workspace host|in your workspace|Paris-proxied|Paris" bob packages/ck-policy/src sanfrancisco/src roma/components/builder-domain.tsx`

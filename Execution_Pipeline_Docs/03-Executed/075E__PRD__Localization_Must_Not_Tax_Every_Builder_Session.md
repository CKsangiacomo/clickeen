...
Priority: P0
Source:
- `Execution_Pipeline_Docs/03-Executed/075__Audit__Authoring_System_Simplification_Findings_And_Slice_Map.md`
...
- post-save translation convergence must be represented by one durable Tokyo/Tokyo-worker overlay work item for the current `publicId + baseFingerprint`
- queue delivery is only the trigger to process that work item; the queue message is not the workflow truth
- post-save translation convergence must be retried by Tokyo/Tokyo-worker from that durable work item; it must not depend on best-effort fire-and-forget callbacks from Roma
...
Tokyo/Tokyo-worker must own one durable overlay work item for the current `publicId + baseFingerprint`.
That work item is the system-owned lifecycle truth for overlay convergence.
Its internal execution states are:

- `pending`
- `running`
- `failed`

Builder then exposes one honest product lifecycle derived from that work item plus ready artifact truth:

- `ok` — every desired locale is current for the current base fingerprint
- `updating` — the current work item is `pending` or `running`
- `failed` — the current work item is `failed`, or desired locales are not ready and the expected work item does not exist

The queue message is not the lifecycle truth.
It is only a trigger to process the durable work item.
...
- Queue delivery is a trigger, not workflow truth. The durable Tokyo/Tokyo-worker work item is workflow truth.
...
- Queue availability and queue retry exhaustion are system/infrastructure concerns. They must surface as honest Tokyo/Tokyo-worker work-item state, not as inline fallback execution and not as infinite `updating`.
...
## Executed Closure - 2026-04-30

`075E` is closed in code.

What changed in this closure pass:

- The Translations panel no longer renders a per-locale readiness/debug list.
- Bob no longer receives or models `failedLocales` from the Roma translations panel route.
- Customer copy no longer tells users to repair translation failures by saving again.
- The panel keeps the intended two visible jobs: translation health and display-locale selection.

Verified current state:

- Translation panel data is loaded lazily only when the panel is open.
- Bob overlay preview uses `readyLocales`.
- Any non-Translations panel keeps preview in editing mode.
- The preview switcher block copy is exactly:
  - `Translations not available while in editing mode. Preview translations in Translations panel.`
- Roma exposes one same-origin route for Bob:
  - `GET /api/account/instances/:publicId/translations`
- That route reads Tokyo translation truth and does not recreate translation truth in Bob.

Verification:

- `./node_modules/.bin/tsc -p bob/tsconfig.json --noEmit`
- `./node_modules/.bin/tsc -p roma/tsconfig.json --noEmit`
- `rg "activeLocales|inspectionLocale|Locale readiness|Translations preparing|Save again to request translation|failedLocales|best available|read-path ensure|ensure.*translation|translation-management|second preview surface" bob/components bob/lib/session roma/components roma/lib/account-instance-translations.ts roma/app/api/account/instances/[publicId]/translations/route.ts`

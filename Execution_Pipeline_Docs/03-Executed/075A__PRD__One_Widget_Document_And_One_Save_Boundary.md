...
Verified and tightened before moving to executed:

- Bob no longer imports or calls `normalizeSessionConfig` on account-mode open/save.
- `bob/lib/session/normalizeSessionConfig.ts` was deleted; the hidden `compiled.defaults` merge path is gone.
- `useSessionBoot` now clones the opened saved config instead of rewriting it.
- `useSessionSaving` now saves the active in-memory widget document directly.
- Roma still writes the saved document through the Tokyo saved-config boundary.
- Roma save now returns save success even if translation follow-up enqueue fails; translation follow-up is reported as follow-up metadata, not as the product meaning of Save.

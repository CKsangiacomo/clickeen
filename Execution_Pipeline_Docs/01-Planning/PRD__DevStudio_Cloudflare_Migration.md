...
- `documentation/architecture/Overview.md` (system map, domain table, Access guidance)
...
2. **AI-operability.** Clickeen's operating thesis is 1 human + AI workforce. A
   ...
   it behind Cloudflare Access makes the internal surface agent-reachable — including
   policy edits, which become reviewable commits instead of silent local disk writes.
3. **It unblocks the local-emulation teardown.** DevStudio-on-localhost is the anchor
   ...
   surface that *requires* that plane to exist. The teardown itself is a follow-up
...
### 3.2 Domains and access

| Environment | Domain | Notes |
| --- | --- | --- |
| Cloud-dev | `devstudio.dev.clickeen.com` | Ships in this PRD |
| Production | `devstudio.clickeen.com` | **Reserved, not deployed** — nothing in the v1 IA reads prod-only state. Deploy only when a future tool needs prod state. |
| Fallback | `devstudio-dev.pages.dev` | Platform default, non-canonical, Access-protected |

Access model (the named boundary):

- **Cloudflare Access with a Clickeen-internal allowlist is the sole authentication.**
- DevStudio never reads, requires, or sets product session cookies. Bob/Roma share
  httpOnly cookies across `*.dev.clickeen.com`; DevStudio ignores them entirely.
  Access membership is not a product role.
- The policy API (§3.5) independently **verifies the Access JWT** on every request —
  the write path is not reachable by anything that merely shares the network path.
...
- **Why commit-to-main, not PR:** pre-GA, single operator, Access-gated — this is
...
- **Auth:** Access JWT verified in the Function (audience check). The GitHub token
  is a Pages secret scoped to this repo. No token, path, or repo coordinates in
  client code.
...
- **Side effect, accepted:** each policy commit to `main` triggers a cloud-dev
  redeploy (Pages Git build). Pre-GA this is acceptable and even desirable — policy
  state and deployed state cannot drift.

Known coupling: the AI-runtime *editor* edits today's matrix schema, which 108A-1
...
| --- | --- | --- | --- |
| 0 | Product owner ratifies: domains (§3.2), cloud-dev-only deployment (prod reserved), Access allowlist membership, the cookie rule, and the write-path design (§3.5 — **ratified 2026-06-09**: commit-to-main, Access-JWT-verified, ck-policy validators reused). | This PRD updated with the decision record (done for write path; remaining items pending). | Starting Step 1 unratified. |
| 1 | Create the Pages project (Git build) for `admin/`; wire `devstudio.dev.clickeen.com`; apply Cloudflare Access to the custom domain and the `pages.dev` fallback. | Unauthenticated request → Access challenge (curl/screenshot); authenticated allowlisted request → app. | Public exposure window; deploying via GitHub Actions; any login UI inside the app. |
...
| 3 | Verify the showcase sections and the Policy Editor's **read** lane on cloud-dev (matrices render from `main`; tiers/limits/flags/AI-runtime values correct). | Screenshot set; spot-check of three entitlement values against `entitlements.matrix.json`. | Wiring writes in this step (that is Step 4). |
| 4 | Build the **cloud write path** (§3.5): Pages Functions for the four routes; GitHub contents-API backend with sha conflict handling; `ck-policy` validators reused; Access JWT verification; typed failures. Page fetch contract unchanged. | On cloud-dev: edit a flag → commit appears on `main` with correct diff and message → page refetch shows the new value. Invalid edit → typed 422, **no commit**. Stale sha → typed conflict. Unauthenticated/cross-origin POST → blocked. **`entitlements.html` hash unchanged (§3.6), or its diff enumerated and product-owner-approved pre-merge.** | Client-side GitHub tokens; bypassing ck-policy validators; silent retries/merges; widening editability beyond today's rules; **any visual change to the editor (§3.6)**. |
| 5 | Docs sync: rewrite `documentation/services/devstudio.md` (cloud model, three-section IA, Access boundary, cookie rule, write-path description, removed-husk note); update the `Overview.md` system map row (DevStudio: Local Vite → Cloudflare Pages, internal). | Docs diff in the same PR as Steps 2–4 code. | Deferring docs; leaving the husk listed as a tool. |
...
- The production deployment (domain reserved only).

## 7. Acceptance criteria

- `devstudio.dev.clickeen.com` serves the three-section app behind Access; the
  `pages.dev` fallback is Access-protected.
- The app sets/reads no cookies other than Cloudflare Access's own.
...
2. **Compliant with architecture/tenets?** Yes — the DevStudio boundary tenets move
   unchanged; Access is the sole gate (JWT-verified at the API); Pages Git build per
   the deploy contract; policy edits become reviewable commits (an improvement over
...
- **A1 (byte-identical lift):** same HTML source file from `admin/src/html/...`, same
  ...
  per page: route loads on cloud-dev, renders without console errors, spot visual
  parity vs local.

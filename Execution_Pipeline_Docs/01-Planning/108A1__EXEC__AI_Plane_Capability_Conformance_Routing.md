# 108A1 — EXEC — AI Plane: Capability Registry, Conformance, Typed Errors, Routing

Status: EXECUTION SPEC v2 — revised 2026-06-09 after the three-perspective
pre-execution review (PM / TPM / Dev Manager); enters `02-Executing` when execution
starts
Owner: San Francisco plane
Parent: `108A__PRD__San_Francisco_AI_Plane_Role_And_Contract.md` (108A-1 slice)
...
interface AgentRoutingPolicy {
  ...
  failover?: Array<{ on: 'AI_PROVIDER_RATE_LIMITED' | 'AI_MODEL_UNAVAILABLE' | 'AI_PROVIDER_TIMEOUT'; to: ModelRef }>;
  copilotMode?: 'full' | 'conversion';          // Q6: derived by Roma at grant mint from
                                                // tier ∧ useful-turn usage (the USAGE_KV
                                                // counter — see 108B1 Step 5b), NOT from
...
// 'guide' is reserved for 108B-2. 'freeform' does not exist: it split into
...
| 1 | Test harness + schema: stand up vitest in `sanfrancisco/` and `packages/ck-policy/` (config files, `src/**/*.test.ts`, wired into the turbo `test` task — today every target package's test script is `echo`). Define `ModelCapabilityProfile` types + registry `packages/ck-contracts/src/model-capabilities.json` with Step 0 candidates (`unverified`); ck-contracts exports the derived provider type. | `pnpm test` runs real tests in both packages; types + registry diff; typecheck green. | Capability fields in the policy matrix; a hand-maintained provider union anywhere. |
...
| 8 | Bob Copilot UI: typed errors render product-safe (copy table from 108B1); picker lists exactly the eligible set. **Ordering note (TPM F-12):** this step edits `CopilotPane.tsx`, which 108B1 Step 3 also edits — execute after 108B1 Step 3 lands, or coordinate the same PR. | UI fixtures; no-raw-JSON e2e assertion; picker e2e. | Error string-matching on provider text. |
...
Durable/service plane contract (108A-2). Workforce agents (108C — translator re-base
happens there, not here; here it is only protected). Copilot behavior (108B1).
Gemini/Mistral onboarding. Closed learning loop (108F). Any SF write to product
truth. Any interim patching of the live Copilot (Q4).

Shipped-reality note carried for 108A-2 (TPM F-13): the workload-budget split is
108A-2 scope, but it is not a *future* problem — the instance-translation queue
...
saturated translation batch can 429 a user's copilot turn now. 108A-2 must treat

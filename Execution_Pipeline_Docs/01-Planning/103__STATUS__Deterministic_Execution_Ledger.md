...
| Local development only | `scripts/dev/generate-berlin-keys.mjs`, `scripts/dev/local-root-env.mjs` | Keep only if clearly marked local-only and absent from product build/deploy/runtime paths. `scripts/dev/local-supabase.mjs` was deleted by the DB Pivot pre-implementation gate because it supported local/remote Supabase target switching. |

...
- **Q3 (amended round 5):** in-domain product questions are the routed `advice`
  ...
  now; Guide (120B-2) is the paid lever later (free = Operator + advice, paid = +
  Guide).
- **Q5 (boundary rule, round-5/6 text):** if a request can land on a control it is
  ...
  your Header title?"); if it is about how Builder works it is Guide (120B-2;
...
| `guide`               | Reserved for 120B-2. Pre-Guide, how-to questions get the Q3 redirect (which may name the panel: "that lives in Layout — I can also just do it: try 'make the pod wider'"). | n/a in this PRD.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
_(DevStudio note: the new key renders generically in its policy editor; its
write-path validators must learn the key.)_
...
`/v1/outcome` path) so 120F can learn which upsell converts per intent.
...
- 120A1 Step 6: plane-side `copilotMode` enforcement + conversion routing rules +
  full-vs-conversion-grant unit test (added there).
- 120B1 **Step 5b** (below): the quota entitlement key + Roma mode derivation +
...
Guide layer (120B-2 — the paid lever). Durable agents (120C). Plane internals
(120A1, parallel). Widget schema changes. Per-widget prompt glue.

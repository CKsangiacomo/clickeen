...
### Use Case 3 — GTM Agent / Durable Orchestrator
...
The SF plane's model selection already lives in the per-agent signed policy. The
architecture supports routing. The implementation decision is which providers to
onboard first.

### Recommended onboarding order:

1. **Gemini 2.5 Flash** → default copilot model
2. **Gemini 2.5 Flash-Lite** → default Babel translation model
3. **Gemini 2.5 Pro** → GTM agent / durable orchestrator
4. **DeepSeek V3** → cost-floor alternative for non-EU copilot workloads
...
_Document prepared June 9, 2026. Prices current as of publication — LLM API pricing is in active competition and should be re-verified quarterly._

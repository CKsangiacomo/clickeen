...
| D-107j | Decide pages-index absence rule: creation-time index materialization or missing-at-read typed failure. | Delete active index truth | 2026-06-12 | Product + Architecture | `accounts/{accountPublicId}/pages/index.json` is not active page truth. Tokyo derives account page list summaries from page `source.json`; page create/save/delete/publish/unpublish do not read, write, heal, or depend on the duplicate pages index. |
| D-107k | Decide whether cache purge failure/stale public serving is deleted in PRD 107 or split to a publish-integrity PRD. | Delete in PRD 107 | 2026-06-13 | Product + Architecture | `TW-107-CACHE-PURGE-IGNORED` remains a deletion target. Missing purge config, missing public serving base, purge request failure, malformed purge response, or unsuccessful purge response must fail the publish/save boundary before success. |
| D-107l | Decide whether generic Tokyo source/package R2 write ordering is a PRD 107 product-truth violation. | Package write plane deleted from save | 2026-06-12 | Product + Architecture | Instance and page source saves no longer write submitted public package bytes. Package readiness is a publish/serve concern; source save is source-only. |
...
| AB-24 | V5/V7 | Tokyo pages | `tokyo-worker/src/domains/pages/source.ts:107-129` | Bad or absent pages index becomes `{ pages: [] }`, then save/delete rewrites the index and erases real summaries. | Tokyo page `source.json` files; account page list is derived from fully validated source files. No stored pages index survives as active truth. | Page list/save/delete/publish routes consuming page source truth. | `saveAccountPageSource`, `writeAccountPagePublicPackage`, `writeAccountPageServeState`, cache purge, `ok: true, pages: []` from duplicate index truth. |
...
| TW-107-PAGE-SAVE-SOURCE-BEFORE-PACKAGE | V6 | Tokyo page save | `tokyo-worker/src/routes/internal-page-routes.ts:240-267`, `domains/pages/source.ts:230-245` | Page save writes source before package write can fail. | Atomic page save transition. | `PUT /__internal/pages/:pageId`. | Page source write before package failure. |
...
| AI-107-AB-19-20 | AB-19, AB-20 | workflow-removal | Page localization/robots payload cannot be silently changed. | Route/boundary failure with structured path; valid page create materializes durable source only, invalid supplied metadata fails before Tokyo create. | COMPLETE |
...
| AI-107-AB-24 | AB-24 | workflow-removal | Corrupt/missing pages index cannot become empty or allow partial mutation. | List/save/delete boundary proof and no source overwrite/delete/package/publish/cache purge. D-107j resolved. | COMPLETE |
...
| --- | --- | --- | --- | --- |
| DNV-107-TW-INSTANCE-SAVE-R2-ORDER | TW-107-INSTANCE-SAVE-SOURCE-BEFORE-PACKAGE | SUPERSEDED. Instance save no longer uses this declared non-violation record; the save-time package write plane was deleted. | Focused source read plus validators must confirm the source-before-package and package-before-source workflows are gone. | SUPERSEDED |
...
| 107-AB17-18-2026-06-12 | 1 | AB-17 / AB-18 | Bob/Roma package-build boundary before Tokyo save | `coreui.errors.widget.packageMissing`; structured `paths` including `product/widgets/countdown/widget.dom.js` | Missing real declared CSS/JS fails before `saveAccountInstanceInTokyo` through Bob compiled package failure. Bob's compiled package now includes declared relative product CSS/JS from widget HTML plus the named shell module contract. Roma requires a compiled `widgetPackage` before save, rejects unstructured compile errors, and no longer refreshes pages after instance save. Tokyo instance save no longer parses or stores submitted package bytes, the orphan Roma public-package builder was deleted, and the 106 migration script no longer builds or writes public package artifacts. | Source/runtime self-audit `32 files changed, 525 insertions(+), 1586 deletions(-)` excluding docs; focused stale-symbol scan found no active `packageSource fallback:`, optional support inference, string-prefix Bob/Roma package transport, post-save `refreshPagesPlacingInstanceForAccount`, `writeAccountPagePublicPackage`, `readSubmittedPagePublicPackage`, `writeInstancePublicPackage`, `readSubmittedInstancePublicPackage`, `roma/lib/widget-public-package.ts`, or migration-script public package write path; `git diff --check`; `pnpm --filter @clickeen/bob typecheck`; `pnpm --filter @clickeen/bob lint`; `pnpm --filter @clickeen/roma typecheck`; `pnpm --filter @clickeen/roma lint`; `pnpm --filter @clickeen/tokyo-worker typecheck`; `pnpm --filter @clickeen/berlin typecheck`; temporary external proof showed missing declared `countdown/widget.dom.js` returns typed `packageMissing` with zero Roma save and valid declared package includes the file; blast-radius validator GREEN; V1-V8 validator GREEN. | this commit |
| 107-AB19-20-2026-06-12 | 1 | AB-19 / AB-20 | Roma page create and account-page direct boundaries | `coreui.errors.page.sourceInvalid` | Page create is restored as a Roma-owned creation boundary: the Create page action submits the initial metadata/display name explicitly, and Roma rejects invalid supplied metadata/robots before Tokyo create. Save/open paths do not trim, coerce, or drop invalid page source fields into a healed page. | Source/runtime self-audit `32 files changed, 525 insertions(+), 1586 deletions(-)` excluding docs; focused stale-symbol scan clean for page package composer and page index mutation paths; `git diff --check`; `pnpm --filter @clickeen/roma typecheck`; `pnpm --filter @clickeen/roma lint`; temporary external proof showed valid page create writes source without pages index, invalid create source writes nothing, and stored JSON `null` source fails invalid rather than being overwritten as absent. | this commit |
...
| 107-AB22-23-2026-06-12 | 1 | AB-22 / AB-23 | Tokyo widget-defaults read/use boundary plus account creation initialization | `tokyo.widgetDefaults.missing`; `tokyo.widgetDefaults.invalid`; `tokyo.widgetDefaults.exists` | Reads remain strict and do not seed. Berlin finish now carries `createdAccount`; Roma calls Tokyo's creation-only widget-default initialization only for that newly created account, and Tokyo writes the durable defaults document only when no defaults document exists. Existing missing/corrupt defaults still fail read/use with no repair write. | Source/runtime self-audit `32 files changed, 525 insertions(+), 1586 deletions(-)` excluding docs; focused read-or-seed/repair/reconstruction scan found no read repair; `git diff --check`; `pnpm --filter @clickeen/tokyo-worker typecheck`; `pnpm --filter @clickeen/roma typecheck`; `pnpm --filter @clickeen/berlin typecheck`; temporary external proof showed missing/corrupt reads fail with zero `put`, creation initialization writes exactly `accounts/{accountPublicId}/widget-defaults.json`, and stored JSON `null` defaults fail invalid rather than being overwritten as absent. | this commit |
| 107-AB24-2026-06-12 | 1 | AB-24 | Tokyo page source list/create/save/delete/publish boundary | `tokyo.errors.page.sourceInvalid`; `tokyo.errors.page.notFound`; `tokyo.errors.page.alreadyExists` | `accounts/{accountPublicId}/pages/index.json` is deleted from active page truth. Page source files own account page list truth; list derives summaries from fully validated `source.json`. Page create writes only the accepted page source plus declared initial unpublished serve-state; save/delete/publish/unpublish validate existing source before downstream mutation. Malformed source fails visibly and triggers no source overwrite/delete/package/cache mutation. | Source/runtime self-audit `32 files changed, 525 insertions(+), 1586 deletions(-)` excluding docs; focused pages-index scan found no active `AccountPagesIndex`, `accountPagesIndexKey`, `indexMissing`, or `indexInvalid` use; `git diff --check`; `pnpm --filter @clickeen/tokyo-worker typecheck`; `pnpm --filter @clickeen/roma typecheck`; temporary external proof showed stale `pages/index.json` is ignored, list derives from source files, valid create writes source without index, and malformed stored/submitted/JSON-null source fails read/save/delete/create with zero unintended puts/deletes. | this commit |
...
| 107-TW-PAGE-SAVE-PACKAGE-2026-06-12 | 4 | TW-107-PAGE-SAVE-SOURCE-BEFORE-PACKAGE | Tokyo page save package mutation plane | n/a - submitted page package save path deleted | Page save no longer accepts, validates, or writes submitted page package bytes. Source save is a source-only mutation; public package readiness remains a publish/serve concern and fails visibly when required files are missing. The string-prefix page package error transport and page package writer were deleted. | Source/runtime self-audit `30 files changed, 425 insertions(+), 1432 deletions(-)` excluding docs; focused scan found no `writeAccountPagePublicPackage`, `readSubmittedPagePublicPackage`, `SubmittedPagePublicPackage`, `page-package-composer`, `roma/lib/widget-public-package.ts`, or `detail.startsWith('page.')`; `git diff --check`; `pnpm --filter @clickeen/tokyo-worker typecheck`; temporary external proof showed invalid source failures block mutation and no page package write plane remains in save. | this commit |
...
| 107-BOB-PREVIEW-MEDIA-2026-06-12 | 4 | PRD107-BOB-006 | Bob Workspace preview media boundary | `Failed to resolve preview media assets` | Failed or unresolved current media asset refs no longer reuse a previous materialized preview state. Workspace posts iframe preview state only after current refs resolve; resolver failure is visible and blocks downstream `ck:state-update` success. | Source/runtime self-audit `1 file changed, 4 insertions(+), 12 deletions(-)` before docs; focused stale-symbol scan found no `lastMaterializedPreviewRef`, stale materialized preview cache, stale-ready condition, or warning-only resolver failure; `git diff --check`; `pnpm --filter @clickeen/bob typecheck`; `pnpm --filter @clickeen/bob lint`; temporary external Playwright proof against bundled Workspace showed valid asset resolution posts materialized media state and marks preview ready, while rejected asset resolution shows the preview asset error and posts zero state updates; blast-radius validator GREEN; V1-V8 validator GREEN. | this commit |
| 107-BOB-PREVIEW-READY-2026-06-12 | 4 | PRD107-BOB-007 | Bob Workspace runtime readiness boundary | n/a - deletion of timeout ready success | Missing widget runtime `ck:ready` no longer becomes ready UI through timeout/fail-safe fallback. Workspace readiness is set only by the iframe `ck:ready` message or visible iframe load failure. | Source/runtime self-audit `1 file changed, 0 insertions(+), 10 deletions(-)` before docs; focused stale-symbol scan found no `visibilityFallbackTimeout`, `readyTimeout`, fail-safe ready timeout, or `setTimeout(() => setIframeHasState(true))` path; `git diff --check`; `pnpm --filter @clickeen/bob typecheck`; `pnpm --filter @clickeen/bob lint`; temporary external Playwright proof against bundled Workspace showed valid iframe `ck:ready` marks preview ready, while missing `ck:ready` remains not ready after the old fallback windows and shows Loading preview; blast-radius validator GREEN; V1-V8 validator GREEN. | this commit |
| 107-DIETER-ASSETS-2026-06-12 | 4 | PRD107-DIETER-002 | Dieter account asset list/upload/resolve boundary | `coreui.errors.assets.payloadInvalid`; `coreui.errors.assets.uploadFailed` | Malformed/defaultable/whitespace-healed list, upload, resolve, and `missingAssetRefs` payloads no longer become partial asset browser or media commit success. Requested refs, stored refs, and committed refs are forwarded/preserved unchanged; generated Tokyo Dieter runtime reflects the same deletion. | Source self-audit `5 files changed, 33 insertions(+), 101 deletions(-)` before docs and generated artifacts; focused stale-symbol scan found no account asset normalizer import/use, survivor list filter, `if (!normalized) continue`, missing-as-empty response shape, local ref filter/dedupe, single-resolve shape fallback, single-resolve trim, dropdown-fill resolve trim, fill-parser account asset-ref normalization, dropdown-upload stored ref normalization, or commit-time asset-ref trim; `git diff --check`; `pnpm --filter @ck/dieter typecheck`; `pnpm --filter @ck/dieter build`; temporary external TS proof showed valid list/upload/resolve succeeds, defaultable list, whitespace list, defaultable upload, whitespace resolve record, missing resolve shape, and whitespace missing ref fail, and requested refs including empty array/whitespace refs are forwarded unchanged; blast-radius validator GREEN after RED fixes; V1-V8 validator GREEN after RED fixes. | this commit |
| 107-ROMA-ASSETS-2026-06-12 | 4 | R107-ROMA-003 / CKC-107-05 | Bob/Dieter/Roma/Tokyo account asset upload, resolve, materialization, and serve boundaries | `coreui.errors.filename.invalid`; `coreui.errors.payload.invalid`; `tokyo.errors.source.invalid`; `tokyo.errors.assets.metadataInvalid`; `coreui.errors.assets.payloadInvalid`; `coreui.errors.assets.resolve.invalidAssetRefs`; `coreui.errors.assets.resolve.invalidMaterialization` | Missing filename/content type/source fails before R2 mutation; malformed stored asset metadata/key fails before list/resolve/serve success; resolve success truth is exactly `{ assets: [{ assetRef, url }] }`; missing requested resolve refs fail instead of returning partial success; malformed media refs or missing resolved truth fail before Bob preview state or Roma save success; standalone Roma asset mutation UI was deleted; public serve no longer guesses missing content type. | Source/runtime self-audit `20 files changed, 287 insertions(+), 864 deletions(-)` before docs and generated artifacts; focused stale-symbol scan found no account asset metadata default, asset-ref alias normalizer, `upload.bin`, `asset.bin`, `application/octet-stream`, content-type guessing, account asset record reconstruction, malformed resolve pass-through, stale preview materialization, or standalone asset mutation workflow in the slice surface; `git diff --check`; `pnpm --filter @ck/dieter typecheck`; `pnpm --filter @clickeen/ck-contracts typecheck`; `pnpm --filter @clickeen/tokyo-worker typecheck`; `pnpm --filter @clickeen/bob typecheck`; `pnpm --filter @clickeen/roma typecheck`; `pnpm --filter @clickeen/bob lint`; `pnpm --filter @clickeen/roma lint`; `pnpm --filter @ck/dieter build`; temporary external TS proof showed valid upload/resolve/serve succeeds, missing filename/content type fail without R2 mutation, missing requested resolve refs fail visibly, bad filename is not sanitized, corrupt stored metadata fails list/resolve/serve even when content type exists, Dieter/Roma reject extra/partial resolved asset success, and deploy/font serve fail missing content type instead of guessing. | this commit |
...
| PRD107-BOB-006 | NO | NO | NO | NO | NO | NO | NO | NO | YES |
| PRD107-BOB-007 | NO | NO | NO | NO | NO | NO | NO | NO | YES |
| PRD107-DIETER-002 | NO | NO | NO | NO | NO | NO | NO | NO | YES |
| R107-ROMA-003 / CKC-107-05 | NO | NO | NO | NO | NO | NO | NO | NO | YES |

...
pivot. Current Widget templates and the Widget Catalog remain product behavior.
Page templates and the Page Catalog are being removed. Future Page execution
requires a new review and rewrite without changing the working Widget model.

Depends on: corrected and verified 127A–127E
...
2. **A template uses the normal object authority plus `isTemplate: true`.** Save
   ...
   JavaScript, Page values, Rows, Row design, settings, or asset references. There is
...
10. **Using a cross-account template makes asset ownership explicit.** If the
    ...
    In PRD 127 this cross-account flow is exercised by Widget Catalog
    templates. The only global Page Catalog template is blank, so cross-account
    Page Row assets are not authorized or implemented yet.
...
call, or row in **Your pages** exists.
...
A Page template remains a Page containing ordered Rows. Each Row retains its
Instance reference plus Page-owned Layout, Style, and Spacing. The template
keeps that complete reusable Page source/config, but its copied config has no `baseLocale`,
...
A same-account Page template keeps valid same-account Page and Row asset
references without a dialog. Catalog-card thumbnails are presentation assets
and are never copied into the customer account.

For same-account Page templates, the new Page retains its Row order, Row design,
and valid same-account Instance references. For the global Page Catalog, 127F seeds only a blank
...
child Instances or Page/Row assets are outside PRD 127. When nonblank global
Page templates are authorized later, that scope must reuse the existing
copy/discard asset authority rather than inventing a Page-template asset system.
...
8. Add the Dieter naming dialog, Template badge, editor state, **Save as
   template** row-menu/editor action locations, **Use template**, and the Dieter
...
15. Correct Page template snapshot/use to consume the 127A `rows` contract,
    retaining Row order and every accepted Row Layout, Style, and Spacing value.
16. Preserve same-account Row-owned background image/video references without
    copying. Do not implement cross-account Page Row assets while the only
    global Page Catalog template is blank; add no Row asset service or template
    package.
17. Remove the former bare-child-reference contract, compatibility aliases,
    tests, and documentation from template and Catalog paths.
...
11. Use the blank Page Catalog template; prove nothing exists before Save and
    ...
    Prove a same-account nonblank Page template retains Row order, Row design,
    and valid same-account Instance references in the unsaved Page draft.
...
21. Prove a same-account Page template with a Row background image/video keeps
    that same-account reference in the unsaved draft without copying or a
    dialog. Prove the blank global Page template has no Row/asset dependency.
...
conditional list-menu and persistent editor action locations; Save as template and
...
and cannot publish; Page templates retain ordered Rows, accepted Row design,
and same-account Row asset references; cross-account Widget Catalog source
assets use the explicit copy/discard choice through the existing asset
authority, while global Page Row dependencies remain outside 127; the old Catalog implementations
...
The evidence below proves the earlier template/Catalog execution. It does not
prove the corrected 127A Page Row contract or Row-owned asset behavior.
...
- Through Page Builder, ordinary Page `KTYKBJWH2G` was saved with zero Widget
  child references. Through DevStudio's normal Catalog route, it was snapshotted as

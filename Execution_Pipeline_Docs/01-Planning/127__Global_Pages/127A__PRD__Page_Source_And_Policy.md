...
pivot. Pages is being removed from the current product. Prague will not migrate
to Pages. Future execution requires a new review and rewrite.
...
- a saved Page containing ordered Rows, each with Page-owned design and one
  reference to a saved same-account Instance;
...
type PageRowLayout = {
  ...
  contentWidth:
    | { mode: 'full' }
    | { mode: 'contained'; maxWidthPx: number };
  minHeightPx?: number;
};

// Same persisted shapes consumed by Dieter Dropdown Fill, Border, and Shadow.
type PageRowStyle = {
...
type PageRowSpacing = {
  desktop: PageRowInsetsPx;
  mobile: PageRowInsetsPx;
};
...
`FillValue`, `BorderValue`, and `ShadowValue` are the existing Dieter persisted
control values. Their product-neutral types must have one shared contract that
both Page source and Dieter consume; 127A must not create similar Page-only
fill, border, or shadow shapes.

The field names, shape, defaults, and ownership above are fixed. Every numeric
Row dimension is a CSS pixel value. Negative widths, heights, and padding are
invalid.

Row geometry is exact:

- the outer Row always spans the available Page width;
- when `fullBleed` is `true`, background/border/shadow apply to that outer Row;
- when `fullBleed` is `false`, the outer Row stays transparent and
  background/border/shadow apply to the inner content wrapper;
- `{ mode: 'full' }` makes the inner content wrapper use all available width;
- `{ mode: 'contained', maxWidthPx }` centers the inner content wrapper at the
  required positive maximum width; `maxWidthPx` does not exist in Full mode;
- when the customer first changes Full to Contained, Page Builder initializes
  the editable maximum width to `1200` pixels, matching the current accepted
  Bob workspace canvas width;
- desktop/mobile padding always applies inside the inner content wrapper.

Neutral Row defaults are `fullBleed: false`, `{ mode: 'full' }`, no minimum
height, `background: { type: 'none' }`,
`border: { enabled: false, width: 1, color: '#c7c7cc' }`,
`shadow: { enabled: false, inset: false, x: 0, y: 8, blur: 24, spread: 0,
color: '#000000', alpha: 18 }`, and zero for all eight desktop/mobile padding
values. These disabled Border/Shadow objects are the current Dieter neutral
objects with the existing edit defaults retained. Adding a Widget always
creates exactly this neutral Row before the customer changes it.

Animation is Row-owned conceptually, but no animation field is part of this
contract until the product owner approves the exact effects, timing, and
controls. Execution must not add a free-form animation object or infer choices
from a competitor screenshot.
...
- no saved optimization report, score, recommendation, or generated finding.
...
| `rows` | Ordered Page Rows. Each Row owns Page layout/style/spacing and references one saved same-account Instance. |
| `rows[].rowId` | Stable Row identity used by Page Builder, generated Page HTML, locale Row values, and optimization findings. |
| `rows[].instanceId` | Saved same-account ordinary Widget Instance rendered inside the Row. |
| `rows[].layout` | Page-owned full-bleed, content-width, maximum-width, and minimum-height choices. |
| `rows[].style` | Existing Dieter fill, border, and shadow values applied outside the Widget. |
| `rows[].spacing` | Page-owned desktop and mobile Row padding. |
...
- keeps the complete reusable Page source/config, including Page values,
  robots choice, Rows, Row design, and direct asset references;
...
The first Clickeen Page Catalog template is blank and has no Rows.
...
The source keeps each Row direct and Page-owned:

```text
rowId + saved same-account instanceId + Row layout/style/spacing
```

- Row order is Page order;
- the referenced Instance must be a saved ordinary Instance owned by the same
  account;
- one Row contains exactly one referenced Instance;
- Row layout, style, and spacing never mutate or override saved Instance
  source; they style the Page-owned wrapper around it;
...
- one Instance ID may appear at most once in one Page;
- a blank Page may be saved, but Publish requires at least one Row.

These Row rules apply to ordinary Pages and account-owned Page
templates. The first Clickeen Catalog Page template has no Rows.

These are ordinary Page input rules, not a new validator framework.

Page Optimization is derived from current saved Page files and is not Page
source. Findings are never written into `source.json`; fixing a finding changes
the owning Page/Row in Page Builder or the owning Widget Instance in Bob and
then requires the normal explicit Save or Update.
...
- [x] Add the former shared `AccountPageSource` and locale-overlay types in one
      named `@clickeen/ck-contracts` Page module; its incomplete bare-reference
      child shape is superseded by the corrective item below.
- [ ] Replace the incomplete bare child-reference contract with the accepted
      `PageRow`/`rows` contract and delete the obsolete shape rather
      than keeping a compatibility alias.
- [ ] Promote the existing persisted Fill/Border/Shadow value types to one
      product-neutral shared contract consumed by both Dieter and Page Rows.
- [ ] Parse, store, and read exact neutral and edited Row values through the
      existing Roma → Tokyo Page authority.
- [ ] Update Page overlay coordinates from obsolete child-reference IDs to Row IDs
      without changing overlay meaning.
...
### 2026-08-05 prior execution evidence

The evidence below proves the former bare-child-reference contract. It does not
close the corrected Row contract above and must not be used to mark 127A green.
...
127A is done when the shared Page source uses ordered Page Rows with the exact
accepted Page-owned design values; no bare-reference compatibility shape
remains; Page overlay meaning, Generate translations, `pages.max`, and Tier99
continue through their existing authorities; current documentation matches the
deployed system; and V1–V8 are independently GREEN.

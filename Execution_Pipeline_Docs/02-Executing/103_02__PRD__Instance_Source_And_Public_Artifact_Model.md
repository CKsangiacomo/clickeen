...
- Translated locale values are `locale -> translated content values`. They are not overlay IDs, selected pointers, version slots, or storage-object identities.
- If Tokyo keeps overlay files internally during migration, they are private exact current value maps only. They do not store provenance, readiness, review state, source hash, generation ID, or manual override status, and they do not appear in product payloads.

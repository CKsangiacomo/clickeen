...
The only product difference for selected admin-owned instances is distribution metadata:

```txt
make this admin-owned instance visible in Roma
allow users to duplicate it into their own account
optionally embed it on Prague
```

That is metadata on a normal account-owned instance. It is not a new storage class and not a new runtime flow.
...
The route can still internally call Tokyo/Venice APIs, but public and code-facing names must stop hiding meaning behind abbreviations.
...
3. locked widget folders cannot create new instances
4. locked widget folders cannot publish
5. locked widget folders cannot be publicly served unless product policy explicitly introduces a grace state
6. upgrade restores access by changing widget-folder state, not by restoring deleted data
...
### Slice 4: Downgrade / Widget Folder Status Enforcement

Implement product handling for widget-type folder state:
...
- fixture account with 3 widget folders can be locked down to 1 active widget type
- no data is deleted on downgrade
- locked widgets are not silently served as normal
...
11. Account downgrade fixture locks widget folders without deleting data
...
15. Prague page translations are stored and named outside account instance overlays

## 9. Non-Goals

This PRD does not introduce:

- multiple account support beyond the account-folder contract
- vanity public URLs
...
Downgrades lock widget type folders, never delete account data.

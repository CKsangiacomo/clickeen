...
| EB-011 | backlog | PRD 075E | Reintroduce the Builder Translations panel only as a simple Tokyo-backed inspector, with future translation correction flowing into Agent Settings / translator guidance instead of per-instance manual overlay editing | `75E` correctly removed always-on localization tax from every Builder session. Bringing the panel back as a second editor, repeater clone, or per-locale manual override system would recreate the same product/architecture mistake and would not scale across many locales and instances. The right future shape is: locales are activated in Roma Settings, background translation is generated by San Francisco and served from Tokyo, Builder uses the panel to answer whether translations are ok and to set selected-locale inspection for the main preview, base locale is not editable in that panel, and any “fix translation” UX must teach the translator system via reusable guidance rather than creating a second persistent translated-output CMS. | Translation review/correction becomes active product work and we open a dedicated PRD that keeps the first reintroduction intentionally small: locale dropdown for the account’s active locales, status visibility, Tokyo-backed translation health for the current instance, preview-locale inspection in the main Builder preview, and at most a targeted “improve translation” modal that captures guidance/corrected wording for future agent behavior instead of freeform per-overlay editing. | — |

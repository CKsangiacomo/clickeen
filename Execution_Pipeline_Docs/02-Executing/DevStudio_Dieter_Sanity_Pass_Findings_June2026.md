...
- **Component-page generator guards are real.** `admin/scripts/generate-component-pages.ts` throws on a missing components dir (`:19`), on an unresolved `{{...}}` stencil marker (`:90-92`), and on a component that renders no page (`:109`). PRD claim CONFIRMED.
...
5. **Masquerade backstop — PARTIAL.** Generation-time guards are real (above), but **no CI drift-detection gate is evident**: the generators run via `admin/package.json`'s `generate` script, yet no `.github/workflows/*` references the generators by name. If `build:dieter` regenerates the committed `admin/src/html/foundations/*.html`, a hand-edit could be silently overwritten or slip through — nothing visibly *fails* on drift. Confirm whether `build:dieter` invokes the generators and whether anything diffs the committed generated files.

...
| `accounts/{accountId}/instances/{instanceId}/instance.config.json` | Tokyo-worker `saved-config.ts` | Non-text config, identity/display metadata, publish status, locale metadata. Read/write product source today. | Stop using as cross-service contract. Keep only behind Tokyo payload operation if still needed physically. DB row owns registry/control state. | 103_DB.2 |
...
| Public files: `index.html`, `styles.css`, `script.js`, `{locale}.html`, `script.{locale}.js` | Tokyo-worker `public-artifacts.ts`, public route | Generated browser artifacts served from R2/CDN. | Keep in R2. Public serving reads these only; app services must not infer authoring state from them. | 103_DB.6 |
...
| Historical `scripts/build-widget-catalog.mjs` | Deleted by 103_01.3b | Former broad bootstrap-era generated catalog writer. | Remains deleted. `scripts/validate-widget-source.mjs` is the non-mutating replacement guard. | 103_DB.5 |
| `scripts/validate-widget-source.mjs` | Widget source validation script | Non-mutating guard that validates widget source files and fails on deleted source drift. | Keep. This is validation, not a generated product-state writer. | 103_DB.5 / guards |

# Strategy PRD - Blueprint Layers + Personalization + Playbooks Execution Plan
...
Note: account intelligence signals (business category, detected country, existing languages) normalized from a `web_presence` connector crawl are a candidate input source for `geo` and `industry` layer key resolution. See `strategy_PRD__Connectors_Universal_Data_Sources.md` section 10 for the web presence connector model. This is additive and does not change the layering contract below.

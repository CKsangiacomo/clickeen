  ...
  <div class="row" data-cols="1" data-token="${entry.name}">
    ...
    <div class="specdpreview">
      ...
      <div class="componentpreview">
        <button class="token-edit-trigger" type="button" data-token-edit="color" data-token="${entry.name}" data-value="${entry.value}" aria-label="Edit ${entry.name}">
...
async function generateIconsPage() {
  ...
  const names = Object.keys(manifest.symbols ?? {}).sort((a, b) => a.localeCompare(b));
  const sizes = [
    ['xxs', '12px'],
    ['xs', '16px'],
    ['sm', '20px'],
    ...
    <div class="icon-row__cells">
      ${sizes
        .map(
          ([size, label]) => `
      <div class="icon-card">
        <span class="icon-card__label">${size} · ${label}</span>

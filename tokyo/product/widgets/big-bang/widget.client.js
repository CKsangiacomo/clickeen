(function () {
  ...
  function normalizeBigBang(state) {
    ...
    const textWidth = typeof bigBang.textWidth === 'number' && Number.isFinite(bigBang.textWidth)
      ? Math.min(Math.max(bigBang.textWidth, 480), 1280)
      : 880;
    const gap = typeof bigBang.gap === 'number' && Number.isFinite(bigBang.gap)
  ...
  function initBigBang(widgetRoot, runtimeContext) {
    ...
    if (!(bigBangRoot instanceof HTMLElement)) throw new Error('[BigBang] Missing [data-role="big-bang"]');
    if (!(coreEl instanceof HTMLElement)) throw new Error('[BigBang] Missing [data-role="big-bang-core"]');
    if (!(statementEl instanceof HTMLElement)) throw new Error('[BigBang] Missing [data-role="big-bang-statement"]');
    if (!(supportEl instanceof HTMLElement)) throw new Error('[BigBang] Missing [data-role="big-bang-support"]');

    if (!window.CKStagePod?.applyStagePod) throw new Error('[BigBang] Missing CKStagePod.applyStagePod');
    window.CKStagePod.applyStagePod(state.stage, state.pod, widgetRoot);

    if (!window.CKTypography?.applyTypography) throw new Error('[BigBang] Missing CKTypography.applyTypography');
    ...
    if (!window.CKCoreSize?.applyCoreSize) throw new Error('[BigBang] Missing CKCoreSize.applyCoreSize');

...
Set by the global Header primitive (`tokyo/product/widgets/shared/header.js`):
- On `.ck-headerLayout`:
...
- Logo tile (generated): `[data-role="logo"]`
  - Visual: `[data-role="logo-visual"]` (uses materialized `logoFill`)
...
- Header richtext is sanitized by `tokyo/product/widgets/shared/header.js`:
...
- Runtime consumes materialized `logoFill`; uploaded logos resolve from `asset.assetId` before runtime config reaches the widget.
...
- It is injected globally by `tokyo/product/widgets/shared/branding.js` into the closest `.pod` as `.ck-branding`.

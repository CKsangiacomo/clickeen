  ...
  function validateItems(cards, treatment) {
    ...
    return rawItems.map(function (raw, index) {
      ...
      const iconName = mediaKind === 'icon'
        ? assertEnum(media.iconName, itemPath + '.media.iconName', CARD_MEDIA_ICON_NAMES)
    ...
    return {
      ...
      betweenCards: {
        ...
        icon: {
          name: assertEnum(
            icon.name,
            'state.cards.betweenCards.icon.name',
            BETWEEN_CARD_ICON_NAMES,
          ),

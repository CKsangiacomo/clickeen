(function () {
  ...
  function cssFill(value, fallback) {
    ...
    if (isRecord(value) && window.CKAppearance && typeof window.CKAppearance.toCssBackground === 'function') {
  ...
  function normalizeItems(state) {
    ...
    if (rawItems.length < 2 || rawItems.length > 16) throw new Error('[Cards] core.items must contain 2-16 cards');
    const seen = {};
  ...
  function normalizeCore(state) {
    ...
    const treatment = ['cards', 'linked-cards', 'steps'].indexOf(core.treatment) >= 0 ? core.treatment : 'cards';
    ...
    return {
      ...
      gap: typeof core.gap === 'number' && Number.isFinite(core.gap) ? Math.min(Math.max(core.gap, 8), 64) : 24,
      cardPadding: typeof core.cardPadding === 'number' && Number.isFinite(core.cardPadding) ? Math.min(Math.max(core.cardPadding, 16), 64) : 32,
      ...
      betweenCards: {
        ...
        line: {
          widthPt: typeof line.widthPt === 'number' && Number.isFinite(line.widthPt) ? Math.min(Math.max(line.widthPt, 1), 24) : 2,
          color: typeof line.color === 'string' && line.color.trim() ? line.color.trim() : '#D7D7DA',
        },
        icon: {
          name: normalizeIconName(icon.name, 'chevron.right'),
          sizePt: typeof icon.sizePt === 'number' && Number.isFinite(icon.sizePt) ? Math.min(Math.max(icon.sizePt, 8), 96) : 28,
          color: typeof icon.color === 'string' && icon.color.trim() ? icon.color.trim() : '#222222',
        },
      },
  ...
  function applyCustomCardStyles(card, item, enabled) {
    ...
    const textTone = ['inherit', 'default', 'muted', 'inverse'].indexOf(item.style.textTone) >= 0 ? item.style.textTone : 'inherit';
  ...
  function renderMedia(item) {
    ...
    if (item.mediaKind === 'icon') {
      ...
      icon.style.setProperty('--ck-cards-icon', 'url("/dieter/icons/svg/' + item.iconName + '.svg")');
  ...
  function renderCard(item, core, state) {
    ...
    if (!window.CKSurface?.applyCardWrapper) throw new Error('[Cards] Missing CKSurface.applyCardWrapper');
  ...
  function renderBetween(core, index) {
    ...
    if (core.betweenCards.kind === 'icon') {
      between.style.setProperty('--ck-cards-between-icon', 'url("/dieter/icons/svg/' + core.betweenCards.icon.name + '.svg")');
      between.style.setProperty('--ck-cards-between-size', core.betweenCards.icon.sizePt + 'pt');
      between.style.setProperty('--ck-cards-between-color', cssFill(core.betweenCards.icon.color, '#222222'));
    } else {
      between.style.setProperty('--ck-cards-between-width', core.betweenCards.line.widthPt + 'pt');
      between.style.setProperty('--ck-cards-between-color', cssFill(core.betweenCards.line.color, '#D7D7DA'));
    }
    return between;
  }
  ...
  function initCards(widgetRoot, runtimeContext) {
    ...
    if (!(coreEl instanceof HTMLElement)) throw new Error('[Cards] Missing [data-role="cards-core"]');

    if (!window.CKStagePod?.applyStagePod) throw new Error('[Cards] Missing CKStagePod.applyStagePod');
    window.CKStagePod.applyStagePod(state.stage, state.pod, widgetRoot);

    if (!window.CKTypography?.applyTypography) throw new Error('[Cards] Missing CKTypography.applyTypography');
    ...
    if (!window.CKCoreSize?.applyCoreSize) throw new Error('[Cards] Missing CKCoreSize.applyCoreSize');

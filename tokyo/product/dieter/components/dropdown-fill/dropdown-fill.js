...
var Dieter = (() => {
  ...
  function readDeclaredString(raw) {
    return typeof raw === "string" && raw && raw === raw.trim() ? raw : null;
  }
  function readOptionalDeclaredString(raw) {
    if (raw === void 0) return "";
    return readDeclaredString(raw);
  }
  function readDeclaredImageValue(raw) {
  ...
  function readDeclaredVideoValue(raw) {
    if (!raw || typeof raw !== "object" || Array.isArray(raw)) return null;
    const value = raw;
    const assetRef = readDeclaredString(value.assetRef);
  ...
  function readDeclaredGradientValue(raw, root) {
    if (!raw || typeof raw !== "object" || Array.isArray(raw)) return null;
    const value = raw;
  ...
  function coerceFillValue(raw, root) {
  ...
  function parseFillValue(raw, root) {
    ...
    try {
      ...
      return coerceFillValue(parsed, root);
    } catch {
      return null;
    }
  }

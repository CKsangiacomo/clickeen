...
export async function handleAccountInstanceRenderSnapshot(
  req: Request,
  env: Env,
  accountId: string,
  publicId: string,
...
export async function handleAccountInstancesList(req: Request, env: Env, accountId: string) {
  const authorized = await authorizeAccount(req, env, accountId, 'viewer');
  if (!authorized.ok) return authorized.response;

  const params = new URLSearchParams({
    select: 'public_id,display_name,status,config,created_at,updated_at,widget_id,account_id,kind',
...
export async function handleAccountInstancePublishStatus(
  req: Request,
  env: Env,
  accountId: string,
  publicId: string,

...
function faqExcerptHtml(args: { state: Record<string, unknown>; locale: string }): string {
...
function faqSchemaJsonLd(args: { state: Record<string, unknown>; locale: string }): string {
  const state = (asRecord(args.state) as unknown) as FaqState;
  const locale = args.locale.trim() || 'en';
...
function countdownExcerptHtml(args: { state: Record<string, unknown>; locale: string }): string {
...
export function generateMetaPack(args: {
  widgetType: string;

...
Prague does not create content. Prague **crawls folders and renders markdown into pages**.

```
tokyo/widgets/*/pages/*.md (English markdown)
    ↓
scripts/prague-build (crawl folders → render blocks)
    ↓
dist/en/**/*.html (English pages)
    ↓
scripts/prague-localize (translate → cache in D1)
    ↓
dist/{locale}/**/*.html (50 locales)
    ↓
Cloudflare Pages (edge-cached)
    ↓
User lands from search → Minibob converts them
```

**Folder structure = page structure. File exists = page exists. No manifest.**
...
## 5) Data source: Markdown files

Prague crawls `tokyo/widgets/{widget}/pages/` and renders whatever files exist.
...
tokyo/widgets/faq/
  ...
  pages/                  # ← Prague crawls this
    landing.md            # → /widgets/faq
    templates.md          # → /widgets/faq/templates
    
    assets/               # Landing + templates assets
      hero.png            # Widget preview for landing
      templates/
        accordion.png
        grid.png
...
for (const file of crawl('tokyo/widgets/*/pages/**/*.md')) {
  ...
  const pageType = getPageType(file.path) // landing | hub | spoke | comparison
  ...
  const html = blocks.map(block => 
    block.render(md.sections, images, { widget, locale: 'en' })
  ).join('')
...
### Scripts (simple, composable)

| Script | Input | Output | Purpose |
|--------|-------|--------|---------|
| `prague-build` | `tokyo/widgets/*/pages/*.md` | `dist/en/**/*.html` | Crawl folders, render English pages |
| `prague-localize` | `dist/en/**/*.html` | `dist/{locale}/**/*.html` | Translate to 50 locales |
| `prague-sitemap` | `dist/**/*.html` | `sitemap-*.xml` | Generate sitemaps + hreflang |
| `prague-deploy` | `dist/` | Cloudflare Pages | Deploy to edge |

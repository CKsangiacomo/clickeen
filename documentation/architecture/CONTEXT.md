...
Tokyo also owns translation generation job state. A Generate click is real only when Tokyo accepts or returns an active generation job. San Francisco must report terminal locale outcomes back to Tokyo; San Francisco telemetry and Bob local state are not product job truth.

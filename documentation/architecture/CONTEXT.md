...
PRD 103_DB_Pivot NOTE: PRD 103 product execution is paused. The active architecture gate is `Execution_Pipeline_Docs/02-Executing/103_DB_Pivot__PRD__Operational_State_In_Supabase_Public_Artifacts_In_R2.md`. Earlier PRD 103_00-103_03 docs moved back to planning because the corrected architecture is deeper than product-operation vocabulary: operational application state belongs in Supabase, public served artifacts belong in Cloudflare R2/CDN, and publish/materialization is the bridge. Do not use the older R2 JSON source model below as a target for new product work.
...
6. **Translations**: translation generation is an explicit Translations-panel product command. Roma calls Tokyo's generate operation; Tokyo resolves current instance content, editable field contracts, target locales, existing translated values, and changed/missing fields, then owns the generation job state while San Francisco performs text production. A new Generate click is fresh intent: Tokyo creates a new current generation and old completions are ignored by job identity. The product result is translated locale values by locale plus Tokyo-owned generation job status. Bob must not infer generation status from local spinner state or translated-locale inventory alone. Tokyo generation truth must not be forced back into an active state by stale registry projection; a generation that never receives terminal San Francisco callbacks must become failed, not poll forever.
...
Tokyo also owns translation generation job state. A Generate click is real only when Tokyo accepts current work; repeated clicks supersede the previous active generation instead of silently resuming stale queue state. San Francisco must report terminal locale outcomes back to Tokyo; San Francisco telemetry, stale registry projection, and Bob local state are not product job truth. Active generation state must have a backend liveness cutoff so a missing consumer or failed callback cannot trap the product in `queued` or `running`.
...
| `editable-fields.json` | Widget-owned editable/translatable field declaration where customer-visible text exists; FAQ is the PRD 103 gold standard |

...
E2E is a small post-deploy smoke layer, not the deployment gate for every
subsystem. Deterministic typechecks, generator contracts, and focused service
tests protect code changes; E2E checks only the shortest real-user boundary
that local checks cannot prove.
...
For a non-Roma deployed surface, use a surface-specific storage state file
and run the owning command for that surface:

```bash
E2E_AUTH_STATE=e2e/.auth/roma-dev.json \
pnpm e2e
```

The Roma default is preserved when `E2E_ROMA_URL` is not set.
...
The browser smoke set above only requires that authenticated access for
`CLICKEEN` works and avoids widget-specific account fixtures.

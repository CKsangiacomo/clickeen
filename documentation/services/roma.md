...
- Roma -> San Francisco calls require explicit `SANFRANCISCO_BASE_URL`; Copilot uses a Roma-minted grant and outcome writes use HMAC signing. Account-instance translation generation is panel-owned and Tokyo-orchestrated: Roma validates the account request and calls Tokyo Generate through the private product-control binding. Tokyo owns changed/missing pickup, queue production, generation state, and translated-locale writes. Roma must not build FAQ translation jobs.
- Roma no longer calls Berlin for a Michael/PostgREST token. Account registry reads/writes go through Berlin product endpoints with the current Berlin bearer, while Tokyo/Tokyo-worker remain the saved-instance source and publish-status boundary.
...
- Roma no longer uses one mixed helper for both Builder-open document loading and broad publish-status lookup. Builder-open reads only the one opened instance's Tokyo publish status for copy-code gating; widgets-domain/account routes use Tokyo product instance operations.
...
- On save, Roma sends the current account command to one Tokyo-worker save transition using `accountPublicId + instanceId`. Tokyo-worker proves the existing saved document, rejects widget-type mismatch, preserves omitted display metadata, writes the saved config/content, and stores the submitted `index.html` / `styles.css` / `runtime.js` package. Translation generation is a separate panel-owned Generate operation routed through Tokyo, not save follow-up.
- Product-path save must stay one user action with one owner in product language: the user saves the instance, Bob/Roma submit the current public package, and Tokyo-worker stores that instance source plus package. Any transport details behind that handoff are implementation residue, not product meaning, and must not turn into a second save mode or a second user-facing workflow.
- Roma no longer exposes a mixed account-instance reader that combines saved document truth with generated/public serving mechanics in one payload. Builder-open remains the document read boundary; widgets-domain flows ask Tokyo product instance operations.
- Widgets-domain account instance identity now comes from Tokyo saved documents. Canonical published/unpublished state is Tokyo's per-instance publish status; browser package files live under `accounts/{accountPublicId}/instances/{instanceId}/` after Builder save and can exist before standalone public serving. The decision to publish, unpublish, delete, downgrade, enforce caps, or repair published-state correctness belongs to Roma/system account operations. Michael status residue must not be treated as surviving authority.
...
- Account duplicate is one Tokyo-worker transition. Tokyo-worker reads the source saved document, mints the new compact 10-character instance ID, and writes the duplicated saved document. Translation generation for the duplicated instance is not implicitly accepted from duplicate; it starts only from the Translations panel Generate operation. Roma does not mint IDs or perform cleanup compensation for Tokyo-owned duplicate state.
...
Roma `widgets` is the product surface that brokers per-instance publish-state changes:
...
- `/widgets` reads account-instance identity and publish status from Tokyo product operations. Michael status residue is not the target architecture.
...
- `Publish` is the explicit serve-on boundary in Widgets: Roma/system account operations apply account containment, tier, cap, and downgrade/suspension checks on the action, then call one Tokyo-worker publish transition. Tokyo-worker performs storage/PBX validation, verifies the stored package files, and marks the instance published; it does not decide account policy or rebuild widget package bytes.
- `Unpublish` calls one Tokyo-worker unpublish transition after Roma/system account operations decide the mutation. Tokyo-worker marks the instance unpublished so `clk.live` stops serving it without deleting saved source or translated locale values; it does not decide whether policy requires unpublish.
- `Delete` is a Roma/system account operation over one account-owned instance. Tokyo-worker removes or tombstones only the addressed `accounts/{accountPublicId}/instances/{instanceId}/` subtree and derived public artifacts as directed by the product operation.
- Downgrade, suspension, tier/cap correction, and other account correctness jobs are Roma/system account operations. Their durable effect is to make account-owned instance state and public serving correctness true before `clk.live` observes them.
...
- Builder save writes the base instance through the Tokyo/Tokyo-worker transition boundary first. Tokyo splits non-text config from user-visible content fields, marks changed content fields for later translation pickup, and returns the saved instance result. Translation work is a separate panel-owned Generate operation forwarded by Roma and owned by Tokyo for changed/missing pickup, queue production, generation state, and translated-locale writes.
- While Builder `Translations` is open, Roma provides Bob a setup snapshot from account context: base locale, plan translation allowance, and active account translations. The translation panel asks Roma to generate missing/changed translations, then polls for translated locale readiness. Locale preview must stay a product-locale operation and must not expose overlay IDs as UI truth.
...
- Roma is **cloud-only** for supported product behavior.
- Local package work is for isolated build/debug only and is not product runtime evidence.

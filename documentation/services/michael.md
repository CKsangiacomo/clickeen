...
Canonical localization overlays for instances (curated + user).

Core columns:
- `public_id` (text) — locale-free instance id
- `locale` (text) — bcp47-ish (`en`, `es`, `es-mx`)
- `ops` (jsonb) — set-only ops generated by agents/manual base overlays
- `user_ops` (jsonb) — per-field manual overrides (set-only ops)
...
Notes:
- Overlay files served by Tokyo are materialized from `ops + user_ops` (user_ops applied last).
- Instance identity remains locale-free; overlays are keyed by `(public_id, locale)`.

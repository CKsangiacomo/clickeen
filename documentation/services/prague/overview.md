...
In this repo, Prague’s widget marketing content is sourced from **checked-in** files under `tokyo/widgets/*/pages/` (not fetched from a remote Tokyo service).
...
- Canonical widget pages load `{page}.json` and render `blocks[]` by `kind`
- Long-tail pages load and parse markdown using a tiny deterministic parser (`prague/src/lib/markdown.ts`)
...
**Result:** Prague generates ~400,000 pages by combining:
- Canonical widget pages (`pages/*.json`)
- Long-tail pages (markdown hubs/spokes/comparisons)
...
| **Pricing** | `/widgets/faq/pricing` | Deprecated in v1 canonical set (use shared pricing) |
...
|-----------|------------------|---------|
| Standard pages | 4 | Overview, Templates, Examples, Features |
...
## 5) Data source (current)

Prague reads `tokyo/widgets/{widget}/pages/` from the repo at build time.

- Canonical widget pages are `{overview|templates|examples|features}.json`.
- Long-tail pages remain markdown-driven (`hubs/`, `spokes/`, `comparisons/`).
...
tokyo/widgets/faq/
  ...
  pages/                  # ← Prague crawls this
    ...
    # Canonical pages (each widget defines its own blocks)
    overview.json         # → /widgets/faq (Overview)
    templates.json        # → /widgets/faq/templates
    examples.json         # → /widgets/faq/examples
    features.json         # → /widgets/faq/features
...
### Markdown schema (long-tail pages only; legacy for canonical pages)

Canonical widget marketing pages (`overview/templates/examples/features`) are now authored as JSON in:
- `tokyo/widgets/{widget}/pages/{overview|templates|examples|features}.json`

Markdown below remains relevant for long-tail pages (hubs/spokes/comparisons) and for migration fallback.
...
**English source of truth. Translations are generated and cached.**

- Canonical widget pages: English JSON (`tokyo/widgets/*/pages/*.json`)
- Long-tail pages: English markdown (`tokyo/widgets/*/pages/**/*.md`)
...
| System | Integration |
|--------|-------------|
| **Tokyo** | Source: `tokyo/widgets/*/pages/*.json` (canonical widget pages) + `tokyo/widgets/*/pages/**/*.md` (long-tail) + i18n catalogs + shared assets |

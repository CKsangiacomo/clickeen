...
| Translation Agent | `widget.instance.translator` | `agents/translation-agent/src/worker.ts` | `agents/translation-agent/wrangler.toml` | Roma generic saved-target translation route/service binding | San Francisco `/model/chat`, Tokyo-worker target-bound translation route | saved Instance or Page locale overlay files in Tokyo/R2 | `pnpm --filter @clickeen/translation-agent eval:translation-agent`, `pnpm e2e:smoke:translation-agent-runtime` |
...
| Translation Agent | Roma saved Instance-or-Page translation operation | writes completed locale overlay files through Tokyo-worker |
...
| Saved Instance or Page source | Roma/Tokyo-worker | Translation Agent receives items built from saved source |

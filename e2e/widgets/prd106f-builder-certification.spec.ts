import { expect, test, type ConsoleMessage, type Frame, type FrameLocator, type Page } from '@playwright/test';
import { hasAuthCookies } from '../helpers/auth-state';

const PRD106F_INSTANCES = [
  { widgetType: 'calltoaction', instanceId: 'SZBSB5HHFJ', corePath: 'calltoaction.action.label', coreKind: 'text' },
  { widgetType: 'big-bang', instanceId: 'QD1G068MX7', corePath: 'bigBang.statement', coreKind: 'text' },
  { widgetType: 'cards', instanceId: 'U37WRSMY7J', corePath: 'cards.items', coreKind: 'cards-json' },
  { widgetType: 'countdown', instanceId: 'H7IF9M2K9B', corePath: 'countdown.timer.labels.days', coreKind: 'text' },
  { widgetType: 'faq', instanceId: 'UZ3JEJSHII', corePath: 'faq.sections', coreKind: 'faq-json' },
  { widgetType: 'logoshowcase', instanceId: '8FMVZFFPJV', corePath: 'logoshowcase.strips', coreKind: 'logos-json' },
  { widgetType: 'split-media', instanceId: 'KUGYTX2ZMQ', corePath: 'splitMedia.alt', coreKind: 'text' },
  {
...
function attachPageErrors(page: Page, collector: ErrorCollector) {
  page.on('console', (message) => {
    ...
    if (message.text().includes("Blocked attempt to show multiple 'beforeunload' confirmation panels")) return;
...
async function expectBuilderPanelReady(bobFrame: FrameLocator, panelName: (typeof MIXED_BUILDER_PANELS)[number]) {
...
async function expectPreviewNonblank(bobFrame: FrameLocator) {
  await expect(bobFrame.locator('section.workspace[data-widget-ready="true"]')).toBeVisible({ timeout: 30_000 });
...
async function waitForSelectedInstance(bobFrame: Frame, timeout = 30_000): Promise<boolean> {
  return bobFrame
    .waitForFunction(() => !document.body.innerText.includes('No instance selected yet'), null, { timeout })
    .then(() => true)
    .catch(() => false);
}
...
async function openBuilderFrame(page: Page, instanceId: string): Promise<Frame> {
  ...
  expect(openResponse.ok(), `builder-open API returned ${openResponse.status()} for ${instanceId}`).toBe(true);

  await expect(page).toHaveURL(new RegExp(`/builder/${instanceId}`));
  const frameHandle = await page.locator('iframe[title="Bob Builder"]').elementHandle({ timeout: 30_000 });
  ...
  if (!instanceSelected) {
    ...
    await expect(retryButton, `builder retry should be available for ${instanceId}`).toBeVisible({ timeout: 5_000 });
    ...
    expect(retryResponse.ok(), `builder-open retry returned ${retryResponse.status()} for ${instanceId}`).toBe(true);
    await expect
  ...
  await expect(bobFrame!.getByRole('button', { name: 'Save' }).first()).toBeDisabled({ timeout: 20_000 });
  return bobFrame!;
}
...
async function saveWidget(page: Page, bobFrame: Frame, instanceId: string) {
  ...
  expect(saveResponse.ok(), `save API returned ${saveResponse.status()} for ${instanceId}`).toBe(true);
}

function buildCoreMutation(instance: (typeof PRD106F_INSTANCES)[number], rawOriginal: string, stamp: string): string {
  ...
  if (instance.coreKind === 'cards-json') {
    ...
    expect(Array.isArray(cards) && cards.length > 0, 'Cards repeater must have at least one item').toBe(true);
  ...
  if (instance.coreKind === 'faq-json') {
    ...
    expect(Array.isArray(sections) && Array.isArray(sections[0]?.faqs) && sections[0].faqs.length > 0).toBe(true);
    sections[0] = {
      ...sections[0],
      faqs: [{ ...sections[0].faqs![0], question: `E2E faq core ${stamp}` }, ...sections[0].faqs!.slice(1)],
    };
  ...
  if (instance.coreKind === 'logos-json') {
    ...
    expect(Array.isArray(strips) && Array.isArray(strips[0]?.logos) && strips[0].logos.length > 0).toBe(true);
    strips[0] = {
      ...strips[0],
      logos: [{ ...strips[0].logos![0], name: `E2E logoshowcase core ${stamp}` }, ...strips[0].logos!.slice(1)],
    };
    return JSON.stringify(strips);
  }

  const visuals = JSON.parse(rawOriginal) as Array<Record<string, unknown>>;
  expect(Array.isArray(visuals) && visuals.length > 0, 'Split carousel media repeater must have at least one item').toBe(
    true,
  );
...
async function expectSavedFieldValue(bobFrame: Frame, path: string, expectedValue: string, isJson: boolean) {
  const field = bobFrame.locator(`[data-bob-path="${path}"]`).first();
  if (!isJson) {
...
test.describe('PRD106F authenticated Builder browser certification', () => {
  ...
  for (const instance of PRD106F_INSTANCES) {
    test(`${instance.widgetType} Builder opens cleanly for ${instance.instanceId}`, async ({ page }) => {
      ...
      await expect(page.getByText('Builder could not open this widget. Please try again.')).toHaveCount(0);
  ...
  for (const instance of PRD106F_INSTANCES) {
    test(`${instance.widgetType} saves and reloads Shell and Core edits for ${instance.instanceId}`, async ({ page }) => {
      test.setTimeout(120_000);
      ...
      try {
        ...
        await bobFrame.getByRole('tab', { name: 'Content' }).click();
        await expect(bobFrame.locator(`[data-bob-path="header.title"]`).first()).toHaveValue(headerValue, {
          timeout: 20_000,
        });
        await expectSavedFieldValue(bobFrame, instance.corePath, coreValue, instance.coreKind !== 'text');
      } finally {
        ...
        await Promise.all(openedPages.slice(1).map((openedPage) => openedPage.close({ runBeforeUnload: false }).catch(() => {})));
      }

...
function faqMarkup(shadowEmbedding: boolean): string {
  const widget = `
  ...
  return shadowEmbedding
    ? `<section><template shadowrootmode="open">${widget}</template></section>`
    : widget;
}

test.describe('Widget runtime behavior', () => {
  ...
  for (const shadowEmbedding of [false, true]) {
    test(`FAQ deep links resolve and write hashes ${shadowEmbedding ? 'inside a shadow root' : 'standalone'}`, async ({
      page,
    }) => {
      const pageErrors: string[] = [];
      page.on('pageerror', (error) => pageErrors.push(error.message));
      await page.setContent(faqMarkup(shadowEmbedding));
      await page.evaluate(() => {

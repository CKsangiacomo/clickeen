...
async function forwardToParis(
  ...
  const url = new URL(
    `${paris.baseUrl.replace(/\/$/, '')}/api/workspaces/${encodeURIComponent(workspaceId)}/instances/${encodeURIComponent(publicId)}/layers/${encodeURIComponent(layer)}/${encodeURIComponent(layerKey)}`
  );

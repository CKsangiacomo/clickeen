...
async function forwardToParis(
  ...
  const url = new URL(
    `${paris.baseUrl.replace(/\/$/, '')}/api/instances/${encodeURIComponent(publicId)}/locales`
  );
...
export async function GET(request: NextRequest, ctx: { params: Promise<{ publicId: string }> }) {
  const { publicId } = await ctx.params;
  if (!publicId) {
    return NextResponse.json({ error: 'INVALID_PARAMS' }, { status: 400, headers: CORS_HEADERS });
  }
  return forwardToParis(request, publicId, { method: 'GET' });
}

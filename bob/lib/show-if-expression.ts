...
function tokenizeShowIf(input: string): Token[] {
  ...
  while (i < src.length) {
    ...
    if (/[0-9]/.test(ch)) {
      ...
      if (!Number.isFinite(value)) throw new Error(`Invalid number "${raw}" in show-if expression`);
...
export function parseShowIfExpression(raw: string): ShowIfAst | null {
  ...
  function parsePrimary(): ShowIfAst {
    ...
    if (tok.t === 'string' || tok.t === 'number' || tok.t === 'boolean' || tok.t === 'null') {
      return { type: 'literal', value: tok.v };
    }
    if (tok.t === 'path') {
      const next = peek();
      if (next && next.t === 'paren' && next.v === '(') {
        if (tok.v !== 'hasLinks') throw new Error(`Unknown show-if function "${tok.v}"`);
        consume();
  ...
  const ast = parseOr();
  if (idx !== tokens.length) throw new Error('Unexpected trailing tokens in show-if expression');
  return ast;
}

export function validateShowIfExpression(raw: string): void {
  parseShowIfExpression(raw);
}
...
function resolveValue(node: ShowIfAst, data: Record<string, unknown>): unknown {
  ...
  return evaluateShowIfExpression(node, data);
}
...
export function evaluateShowIfExpression(node: ShowIfAst, data: Record<string, unknown>): boolean {
  switch (node.type) {
    case 'path':
    ...
    case 'call':
      return Boolean(resolveValue(node, data));
    case 'not':
      return !evaluateShowIfExpression(node.child, data);
    case 'and':
      return evaluateShowIfExpression(node.left, data) && evaluateShowIfExpression(node.right, data);
    case 'or':
      return evaluateShowIfExpression(node.left, data) || evaluateShowIfExpression(node.right, data);
    case 'eq':
      return resolveValue(node.left, data) === resolveValue(node.right, data);
    case 'neq':
      return resolveValue(node.left, data) !== resolveValue(node.right, data);
  }
}

...
export async function resolveSessionBearer(request: NextRequest): Promise<SessionResolution> {
  ...
  return {
    ...
    setCookies: [
      { name: ACCESS_COOKIE, value: refreshed.accessToken, maxAge: refreshed.accessTokenMaxAge },
      { name: REFRESH_COOKIE, value: refreshed.refreshToken, maxAge: refreshed.refreshTokenMaxAge },
    ],
  };
}

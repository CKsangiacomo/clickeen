...
${EMBED_LOCALE_RUNTIME_SOURCE}
        ...
        const boot = async () => {
          const instanceId = parseInstanceIdFromPath();
          if (!instanceId) {
            showError('Clickeen embed error', 'Missing instanceId in URL.', window.location.pathname);
            return;
          }
          ...
          const pointerRes = await fetch('/renders/widgets/' + encodeURIComponent(instanceId) + '/live/r.json', {
            method: 'GET',
            cache: 'no-store',
            credentials: 'omit',
          });
          if (!pointerRes.ok) {
            showError('Widget unavailable', 'This widget is not live.', 'instanceId=' + instanceId);
            return;
          }

          const pointer = await pointerRes.json().catch(() => null);
          if (!pointer || typeof pointer !== 'object') {
            showError('Widget unavailable', 'Invalid runtime pointer.', 'instanceId=' + instanceId);
            return;
          }
          ...
          if (!widgetType || !configFp) {
            showError('Widget unavailable', 'Missing widget metadata.', 'instanceId=' + instanceId);
            return;
          }

          const configRes = await fetch(
            '/renders/widgets/' + encodeURIComponent(instanceId) + '/config.json',
            {
              method: 'GET',
              cache: 'force-cache',
              credentials: 'omit',
          ...
          if (!configRes.ok || !configPack) {
            showError('Widget unavailable', 'Config missing.', 'instanceId=' + instanceId);
            return;
          }
          ...
          if (!baseState) {
            showError('Widget unavailable', 'Config invalid.', 'instanceId=' + instanceId);
            return;
          }
          ...
          const textPack =
            overlay && typeof overlay === 'object' && overlay.textPack && typeof overlay.textPack === 'object'
              ...
              : locale === baseLocale
                ? {}
                : null;
          if (!overlayRes.ok && locale !== baseLocale) {
            showError('Widget unavailable', 'Text overlay missing.', 'instanceId=' + instanceId + ' locale=' + locale);
            return;
          }
          if (!textPack) {
            showError('Widget unavailable', 'Text overlay invalid.', 'instanceId=' + instanceId + ' locale=' + locale);
            return;
          }

          applyTextOverrides(baseState, textPack);

          window.CK_WIDGET = { instanceId, locale, state: baseState };
...
export async function GET(_req: Request, _ctx: { params: Promise<{ instanceId: string }> }) {

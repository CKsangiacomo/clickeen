...
function renderPath(accountPublicId: string, instanceId: string, suffix: string): string {
  return `/renders/accounts/${encodeURIComponent(accountPublicId)}/instances/${encodeURIComponent(instanceId)}${suffix}`;
}

export async function GET(req: Request, ctx: { params: Promise<{ accountPublicId: string; instanceId: string }> }) {
  const context = createVeniceRequestContext(req);
  const { accountPublicId, instanceId } = await ctx.params;
  ...
  if (!isCompactAccountPublicId(accountPublicId)) {
    return finish(
      errorResponse(
        ...
        'Invalid accountPublicId in URL.',
        `accountPublicId=${accountPublicId}`,
        400,
      ),
      'coreui.errors.accountPublicId.invalid',
      'invalid_account_public_id',
    );
  }
  ...
  const pointerRes = await tokyoFetch(renderPath(accountPublicId, instanceId, '/live/r.json'), {
    method: 'GET',
  ...
  const configRes = await tokyoFetch(renderPath(accountPublicId, instanceId, '/config.json'), {
    method: 'GET',
    headers: upstreamHeaders,
  });
  ...
  if (locale !== policy.baseLocale) {
    ...
    const overlayRes = await tokyoFetch(renderPath(accountPublicId, instanceId, `/overlays/${encodeURIComponent(overlayId)}.json`), {
      method: 'GET',
      headers: upstreamHeaders,
    });
  ...
  const runtimeScript = buildRuntimeScript({ accountPublicId, instanceId, locale, state, localeLabels, localePolicy: policy });

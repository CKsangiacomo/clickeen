...
export function renderTestimonialsPage({
  instance,
  theme,
  device,
  backlink,
  ...
  const title = typeof (cfg as any).title === 'string' ? (cfg as any).title : 'What our customers say';
  const items = Array.isArray((cfg as any).items)
    ? (cfg as any).items as Array<{ quote: string; author?: string; role?: string }>
    : [];
  ...
  const grid = items.slice(0, 6).map((it) => `
    <figure class="t">
      <blockquote>“${escapeHtml(it.quote)}”</blockquote>
      <figcaption>${escapeHtml(it.author ?? '')}${it.role ? `, ${escapeHtml(it.role)}` : ''}</figcaption>
    </figure>`).join('');
...
<html lang="en" data-theme="${escapeHtml(theme)}" data-device="${escapeHtml(device)}">
  <head>
    ...
    <style nonce="${escapeHtml(nonce)}">
      ...
      .t { border:1px solid ${border}; border-radius:12px; padding:12px; background:transparent; }
      blockquote { margin:0 0 6px; }
      figcaption { font-size:12px; opacity:0.8; }
  ...
  <body>
    <div class="wrap">
      <article class="card" role="region" aria-label="Testimonials">

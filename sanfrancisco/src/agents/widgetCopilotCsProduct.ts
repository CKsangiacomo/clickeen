...
export function looksLikeExplainIntent(prompt: string): boolean {
  return /\b(explain|what can you (do|change)|what are you able to change|what can i ask|how does this work)\b/i.test(
    prompt,
  );
}
...
export function maybeClarifyCs(args: {
  ...
  if (
    /\b(adjust|change|update|make)\b[\s\S]{0,30}\b(everything|all of it|the whole thing|all settings)\b/i.test(
      prompt,
    )
  ) {
    return (
  ...
  const backgroundConcept =
    args.dict.concepts.find((concept) => concept.id === "background") ?? null;
  if (backgroundConcept && containsAny(prompt, backgroundConcept.synonyms)) {
    const hasStageBackground = args.input.controls.some(
      (c) =>
        inferScopeFromPath(c.path) === "stage" && c.path.includes("background"),
    );
    const hasPodBackground = args.input.controls.some(
      (c) =>
        inferScopeFromPath(c.path) === "pod" && c.path.includes("background"),
    );
    if (hasStageBackground && hasPodBackground) {
      return (
        args.dict.clarifications.find(
          (entry) => entry.conceptId === "background",
        )?.question ??
  ...
  const fontConcept =
    args.dict.concepts.find((concept) => concept.id === "font") ?? null;
  if (fontConcept && containsAny(prompt, fontConcept.synonyms)) {
    return (
      args.dict.clarifications.find((entry) => entry.conceptId === "font")
        ?.question ??
...
export function resolveCsPrelude(args: {
  ...
  message: string;
  usageModel: string;
  intent: "explain" | "clarify";
} | null {
  if (
    args.conversationLanguage === "en" &&
    looksLikeExplainIntent(args.input.prompt)
  ) {
...
export function finalizeCsOps(args: {
  ...
  if (
    ...
    message =
      ...
      "Choose a tone for the rewrite (professional, friendly, or conversion-focused), and I will update all editable content fields in one pass.";

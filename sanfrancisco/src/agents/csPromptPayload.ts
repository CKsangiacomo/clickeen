...
function expandControlPathPattern(args: {
  root: unknown;
  pathPattern: string;
  maxPaths?: number;
}): string[] {
  ...
  const maxPaths =
    typeof args.maxPaths === "number" && args.maxPaths > 0
      ? Math.floor(args.maxPaths)
      : 120;
...
function isContentControl(control: ControlSummary): boolean {
  ...
  return /question|answer|title|subtitle|cta|label|text|content|description/.test(
    text,
  );
}
...
function isContentRewriteIntent(prompt: string): boolean {
  ...
  const hasContentNoun =
    /\b(question|questions|answer|answers|title|subtitle|section|cta|copy|text|content)\b/.test(
      text,
    );
  if (!hasContentNoun) return false;
  const hasEditVerb =
    /\b(rewrite|rephrase|refresh|improve|update|edit|change|adapt|translate|localize|polish|simplify|shorten|expand)\b/.test(
      text,
    );
...
function buildConcreteValueRows(args: {
  ...
  const valueMaxLen =
    typeof args.valueMaxLen === "number" && args.valueMaxLen > 0
      ? Math.floor(args.valueMaxLen)
      : 96;
...
export function buildCsPromptPayload(input: CsPromptInput): string {
  ...
  const valueRows = rewriteLike
    ...
    : buildConcreteValueRows({
        controls: selectedControls.filter(
          (control) => !/shadow|border|radius|opacity|blur/i.test(control.path),
        ),
        currentConfig: input.currentConfig,
        maxRows: 28,
        valueMaxLen: 96,
  ...
  if (rewriteLike) {
    while (
      payload.length > MAX_PROMPT_CHARS &&
      trimmedControlRows.length > 18
    ) {

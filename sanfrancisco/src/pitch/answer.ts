...
async function openaiAnswerFromSources(args: { env: Env; model: string; locale?: string; question: string; sources: PitchCitation[] & any[] }) {
...
export async function handlePitchAnswer(request: Request, env: Env): Promise<Response> {
  ...
  if (!index) throw new HttpError(500, { code: 'PROVIDER_ERROR', provider: 'vectorize', message: 'Missing PITCH_DOCS binding' });
  ...
  // Hydrate citation metadata so UIs can render links.
  ...
  const out: PitchAnswerResponse = { answer, citations: hydrated };

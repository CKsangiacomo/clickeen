...
export function WidgetList({
  ...
  const saveAsTemplateDialogRef = useRef<HTMLDialogElement>(null);
  const saveAsTemplateLifecycleRef = useRef<DialogLifecycle | null>(null);
  ...
  useEffect(() => {
    const dialog = saveAsTemplateDialogRef.current;
    if (!dialog) return;
    const lifecycle = createDialogLifecycle({
      dialog,
      initialFocus: () => dialog.querySelector('input'),
      requestDismiss: () => setSaveAsTemplateInstance(null),
    });
    saveAsTemplateLifecycleRef.current = lifecycle;
    return () => lifecycle.destroy();
  }, []);

  useEffect(() => {
    if (saveAsTemplateInstance) saveAsTemplateLifecycleRef.current?.open();
    else saveAsTemplateLifecycleRef.current?.close();
  ...
  const handleSaveAsTemplate = useCallback(async () => {
    ...
    setTemplateCount(null);
    try {
      const templates = await loadRomaWidgetTemplates({
        accountId: productAccountId,
        fetchJson: accountApi.fetchJson,
      });
  ...
  return (
    <>
      {widgetDataError || mutationError || renameError || (domainLoading && widgetInstances.length === 0) ? (
        <section className="rd-canvas-module" role={widgetDataError || mutationError || renameError ? 'alert' : 'status'}>
          ...
          {domainLoading && widgetInstances.length === 0 && !widgetDataError ? <p className="body-m">Loading widgets...</p> : null}
        </section>
      ) : null}
            ...
            >
              ...
              {canSaveAsTemplate ? (
                <button
                  ...
                  onClick={() => {
                    closeWidgetActions();
                    openSaveAsTemplate(openWidgetActionsInstance);
                  }}
                >
      ...
      <dialog
        ...
        data-size="medium"
        aria-labelledby="roma-save-template-title"
      >
        <header className="diet-popup__header">
          <h2 id="roma-save-template-title" className="heading-4">Save as template</h2>
        </header>
        <div className="diet-popup__body">
          {createdTemplate ? <p className="body-m">{createdTemplate.templateName} was saved as a template.</p> : <><DieterTextfield
            label="Template name"
            value={templateName}
            maxLength={120}
          ...
          {saveAsTemplateError ? <p className="body-s" role="alert">{saveAsTemplateError}</p> : null}</>}
        </div>
        <footer className="diet-popup__footer">
          <div className="diet-popup__actions">
            {createdTemplate ? <><button
              className="diet-btn-txt"
              data-size="md"
              data-variant="neutral"
              type="button"
            ...
            >
            ...
            </button><button className="diet-btn-txt" data-size="md" data-variant="primary" type="button" onClick={() => router.push(`/builder/${encodeURIComponent(createdTemplate.templateId)}`)}><span className="diet-btn-txt__label body-m">Open template</span></button></> : <><button className="diet-btn-txt" data-size="md" data-variant="neutral" type="button" onClick={closeSaveAsTemplate} disabled={Boolean(activeActionKey?.startsWith('save-template:'))}><span className="diet-btn-txt__label body-m">Cancel</span></button><button
              className="diet-btn-txt"
              data-size="md"
              data-variant="primary"
              type="button"
              ...
              disabled={
                !canSaveAsTemplate ||
                !templateName.trim() ||
                templateName.trim() === (saveAsTemplateInstance?.displayName || DEFAULT_INSTANCE_DISPLAY_NAME).trim() ||
                Boolean(activeActionKey)
              }
            >
              <span className="diet-btn-txt__label body-m">
                {activeActionKey?.startsWith('save-template:') ? 'Saving...' : 'Save as template'}
              </span>
            </button></>}
          </div>
        </footer>

...
function hasSamePlacementOrder(left: PagePlacementDraft[], right: PagePlacementDraft[]): boolean {
  return left.length === right.length && left.every((placement, index) => placement.placementId === right[index]?.placementId);
}
...
export function PageBuilderContent({
  ...
  const [orderDiscardOpen, setOrderDiscardOpen] = useState(false);
  ...
  const loadInstances = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await loadRomaWidgetsForAccount({ accountId: accountContext.accountPublicId, fetchJson: accountApi.fetchJson });
      setInstances(response.instances);
    } catch {
      setError('Widgets could not be loaded. Please try again.');
    } finally {
      setLoading(false);
    }
  }, [accountApi.fetchJson, accountContext.accountPublicId]);

  requestOrderDismissRef.current = () => {
    if (hasSamePlacementOrder(orderDraft, placements)) {
      setOrderOpen(false);
      return;
    }
    setOrderOpen(false);
    setOrderDiscardOpen(true);
  };

  useEffect(() => {
    if (!addOpen) return;
    void loadInstances();
  }, [addOpen, loadInstances]);
  useEffect(() => {
    const addDialog = addDialogRef.current;
    const orderDialog = orderDialogRef.current;
    if (!addDialog || !orderDialog) return;
    const addLifecycle = createDialogLifecycle({ dialog: addDialog, initialFocus: () => addDialog.querySelector('button'), requestDismiss: () => onAddOpenChange(false) });
    const orderLifecycle = createDialogLifecycle({ dialog: orderDialog, initialFocus: () => orderDialog.querySelector('button'), requestDismiss: () => requestOrderDismissRef.current() });
  ...
  useEffect(() => {
    placementRowsRef.current.get(selectedPlacementId)?.scrollIntoView({ block: 'nearest', inline: 'nearest' });
  }, [selectedPlacementId]);
    ...
    <section className="roma-page-panel" aria-labelledby="page-content-title">
      <div className="roma-page-panel__header">
        ...
        <div className="roma-page-panel__actions">
          <button className="diet-btn-txt" data-size="sm" data-variant="secondary" type="button" onClick={() => onAddOpenChange(true)}><span className="diet-btn-txt__label body-s">Add widget</span></button>
      ...
      <div className="roma-page-placement-list">
        {placements.map((placement) => (
          <div key={placement.placementId} ref={(row) => { if (row) placementRowsRef.current.set(placement.placementId, row); else placementRowsRef.current.delete(placement.placementId); }} className="roma-page-placement-row" aria-current={selectedPlacementId === placement.placementId ? 'true' : undefined}>
        ...
        {!placements.length ? <div className="roma-page-panel__empty"><p className="body-m">This page has no widgets yet.</p><button className="diet-btn-txt" data-size="md" data-variant="primary" type="button" onClick={() => onAddOpenChange(true)}><span className="diet-btn-txt__label body-m">Add widget</span></button></div> : null}
      </div>
      ...
      <dialog ref={addDialogRef} className="diet-popup roma-page-widget-picker" data-size="large" aria-labelledby="add-widget-title">
        ...
        <div className="diet-popup__body">
          {error ? <div><p className="body-m" role="alert">{error}</p><button className="diet-btn-txt" data-size="sm" data-variant="secondary" type="button" disabled={loading} onClick={() => void loadInstances()}><span className="diet-btn-txt__label body-s">Retry</span></button></div> : null}
        ...
        <footer className="diet-popup__footer"><div className="diet-popup__actions"><button className="diet-btn-txt" data-size="md" data-variant="secondary" type="button" onClick={() => onAddOpenChange(false)}><span className="diet-btn-txt__label body-m">Close</span></button></div></footer>
      </dialog>
      ...
      <dialog ref={orderDialogRef} className="diet-popup" data-size="medium" aria-labelledby="manage-order-title">
        ...
        <footer className="diet-popup__footer"><div className="diet-popup__actions"><button className="diet-btn-txt" data-size="md" data-variant="secondary" type="button" onClick={() => requestOrderDismissRef.current()}><span className="diet-btn-txt__label body-m">Cancel</span></button><button className="diet-btn-txt" data-size="md" data-variant="primary" type="button" onClick={() => { onChange(orderDraft); setOrderOpen(false); }}><span className="diet-btn-txt__label body-m">Save</span></button></div></footer>
      </dialog>
      <RomaUnsavedChangesDialog
        open={orderDiscardOpen}
        message="Your reorder and remove changes have not been saved."
        onKeepEditing={() => { setOrderDiscardOpen(false); setOrderOpen(true); }}
        onDiscard={() => { setOrderDraft([...placements]); setOrderDiscardOpen(false); }}
      />
    </section>
  );
}

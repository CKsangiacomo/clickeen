...
export async function DELETE(request: NextRequest, context: RouteContext) {
  ...
  const { accountId: accountIdRaw, invitationId: invitationIdRaw } = await context.params;
  ...
  try {
    ...
    const upstream = await fetch(
      `${berlinBase}/v1/accounts/${encodeURIComponent(accountId)}/invitations/${encodeURIComponent(invitationId)}`,
      {
        method: 'DELETE',
        headers: {

...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  if (!isUuid(memberId)) {
    return withSession(
      request,
      NextResponse.json(
        { error: { kind: 'VALIDATION', reasonKey: 'coreui.errors.account.memberId.invalid' } },
        { status: 422 },
      ),
      session.setCookies,
    );
  ...
  try {
    ...
    const upstream = await fetch(
      `${berlinBase}/v1/accounts/${encodeURIComponent(accountId)}/members/${encodeURIComponent(memberId)}/profile`,
      {
        method: 'PATCH',
        headers: {

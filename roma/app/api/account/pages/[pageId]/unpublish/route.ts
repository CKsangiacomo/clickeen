...
import { unpublishAccountPage } from '@roma/lib/account-pages';
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  const access = resolvePageProductPolicy(current.value.authzPayload, 'unpublish_page');
  if (!access.ok) {
  ...
  const result = await unpublishAccountPage({

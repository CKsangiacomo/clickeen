...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  const bodyResult = await readJsonPayloadOrValidation<{ baseLocale?: string } | null>(request);
  if (!bodyResult.ok) {
    return withSession(
      request,
  ...
  const result = await loadAccountInstanceLocaleOverlayInventory({
    accountId,
    instanceId,
    baseLocale: typeof bodyResult.payload?.baseLocale === 'string' ? bodyResult.payload.baseLocale : '',

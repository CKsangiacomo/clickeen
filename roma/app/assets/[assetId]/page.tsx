...
export default async function AssetDetailPage({ params }: AssetPageProps) {
  ...
  return (
    <RomaShell activeDomain="assets" title={`Asset ${assetId}`} headerRight={<RomaShellDefaultActions />}>
      <section className="rd-canvas-module" aria-label="Asset detail placeholder">
        <p className="body-m">Next step: connect used-by graph and guard destructive actions through policy-aware endpoint contracts.</p>
        <div className="rd-canvas-module__actions">
          <Link href="/assets" className="diet-btn-txt" data-size="md" data-variant="primary">
            <span className="diet-btn-txt__label body-m">Back to assets</span>
          </Link>
        </div>
      </section>
    </RomaShell>

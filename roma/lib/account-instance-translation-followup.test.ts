...
test('Roma save follow-up translates one changed FAQ answer and writes complete current language values', async () => {
    ...
    env: {
      TOKYO_PRODUCT_CONTROL: {
        async fetch(input: RequestInfo | URL, init?: RequestInit) {
          ...
          if (url.pathname === `/__internal/overlays/${NEXT_OVERLAY_ID}.json`) {
            return jsonResponse({
              v: 1,
              overlayId: NEXT_OVERLAY_ID,
              values: tokyoWrites[0]?.values ?? {},
            });
          }
          return jsonResponse(
            {
              error: {
                reasonKey: 'unexpected_tokyo_path',
                detail: url.pathname,
              },
            },
            500,
  ...
  try {
    ...
    if (originalContext === undefined)
      delete (globalThis as Record<PropertyKey, unknown>)[CLOUDFLARE_REQUEST_CONTEXT_SYMBOL];
...
test('Roma generate translations fails when Tokyo cannot read back the written overlay', async () => {
  const currentConfig = {
    header: { title: 'FAQs', subtitleHtml: 'Quick answers' },
    cta: { label: 'Book now' },
    sections: [],
        ...
        async fetch(input: RequestInfo | URL) {
          ...
          if (url.pathname === `/__internal/overlays/${NEXT_OVERLAY_ID}.json`) {
            return jsonResponse(
              {
                error: {
                  reasonKey: 'tokyo.errors.l10n.incomplete',
                  detail: 'overlay_incomplete',
                },
              },
              422,
            );
  ...
  try {
    const result = await runInstanceTranslationFollowupAfterSave({
      ...
      previousConfig: null,
      translateAllCurrentFields: true,
      requestId: 'req_103_generate_verify_failure',
    });

...
interface Config {
  ...
  authBearerOverride?: string;
  redisUrl?: string;
  withRedisTest: boolean;
}
...
async function prepareContext(config: Config): Promise<FixtureContext> {
...
function defaultSupabaseHeaders(config: Config): Record<string, string> {
  return {
...
async function supabaseRequest(url: string, init: RequestInit, errorMessage: string) {
...
async function startLocalServices(config: Config): Promise<ChildProcess[]> {
  const processes: ChildProcess[] = [];

  const spawnDev = (filter: string, extraEnv: Record<string, string> = {}) => {
...
async function waitForHttp(url: string, name: string, options?: { attempts?: number; delayMs?: number; acceptStatus?: (status: number) => boolean }) {
  const attempts = options?.attempts ?? 60;
  const delayMs = options?.delayMs ?? 1000;
  const acceptStatus = options?.acceptStatus ?? ((status: number) => status >= 200 && status < 500);
...
async function runAndRecord(
  test: {
    id: number | string;
    name: string;
    run: () => Promise<{ status?: 'pass' | 'skipped'; metrics?: Record<string, unknown> }>;
    optional?: boolean;
  },
  results: TestResult[],
) {
...
async function testCspNonce(config: Config, context: FixtureContext) {
...
async function testCorsS2S(config: Config, context: FixtureContext) {
...
async function testCorsDisallowed(config: Config, context: FixtureContext) {
  const res = await fetch(`${config.baseParis}/api/instance/${encodeURIComponent(context.publicId)}`, {
    method: 'PUT',
    headers: {
      Authorization: `Bearer ${context.token}`,
...
async function testPutGuard(config: Config, context: FixtureContext) {
  const res = await fetch(`${config.baseParis}/api/instance/${encodeURIComponent(context.publicId)}`, {
    method: 'PUT',
    headers: {
      Authorization: `Bearer ${context.token}`,
...
async function testPutStatusOnly(config: Config, context: FixtureContext) {
  const res = await fetch(`${config.baseParis}/api/instance/${encodeURIComponent(context.publicId)}`, {
    method: 'PUT',
    headers: {
      Authorization: `Bearer ${context.token}`,
...
async function testSubmissionDedup(config: Config, context: FixtureContext) {
  const submission = {
    fields: {
...
async function testRedisBreaker(config: Config) {
...
async function testDbIndexes(config: Config) {
